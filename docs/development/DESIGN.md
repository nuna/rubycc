...
- **R5. コンパイラ単体では不十分 — ツールチェイン全体が必要**
  ...
  - **make 互換ツール(rmake)**: mkmf が生成する Makefile を処理できるサブセット。
    ミニマム環境には /bin/sh が無いため、レシピをシェル無しで実行できること。
  - **シェルのサブセット(`Rubycc::Shell`)**: レシピが書かれている言語は sh なので、
    make だけでは足りない。**単純コマンド・`&&` / `||` / `;`・リダイレクトに加えて、
    Automake / libtool が生成する制御構造(`for` / `if` / brace group)とシェル変数**を
    自前で解釈する(`rmake-shell-subset-1`)。**実行ファイルとしては提供しない** —
    シェルを起こす側(Ruby の `system`、`#!/bin/sh`)はパスが焼き込まれており、
    別名の実行ファイルは誰からも指されないため、rmake がインプロセスで呼ぶ層に留める。
    パイプ・コマンド置換・`while` は範囲外で、明示的に拒否する。
    **`sed` / `awk` 等のテキスト処理ツールは提供しない**(それらに依存するレシピは
    対象外。実測は `rmake-shell-subset-1`)。
...
- **R7. C 言語仕様のサポート範囲の明確化**
  ...
  - それでも必要な最小限の拡張:
    ...
    - `__builtin_expect`, `__builtin_alloca`, `__builtin_va_*`, 主要な `__builtin_*`
      ...
      [`docs/internals/IR.md` §6.5](IR.md#65-ターゲット別の実装範囲)に定める)。
...
- **N1. コンパイル速度**: 典型的な gem(数千〜数万行)を数十秒以内でビルドできること。
  巨大な単一 TU(sqlite3 amalgamation ≒ 25 万行)は「動くが遅い」を許容し上限を設けない。
  **行/秒は受け入れ条件ではなく、回帰を検知するための継続計測値**である
  (2026-08-27 にこう改めた。当初は「YJIT 有効時に目安 20,000 行/秒以上(前処理後)」を
  併記していた。経緯と、行/秒が明確に下がったときに何をするかは
...
- **共有ライブラリのシンボル介入(interposition)**: 自分で定義し自分で参照する
