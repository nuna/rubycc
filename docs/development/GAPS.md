...
| **X**([issue](../../issues/locale-dependent-file-reads.md)) | **ソース以外の読み出しがロケール依存**。`.pc` / Makefile / Gemfile / リンカスクリプトを `File.read` で読む 4 か所 | **`LANG` 未設定の環境で落ちる**。`.pc` の非 ASCII は実在し、Gemfile は UTF-8 コメント入りが普通。`rubycc-doctor` は導入判断で最初に叩かれる | **2 件は実測**(2026-08-20、`.pc` と Gemfile)。残る 2 か所は同じ形だが未測定 | ソース側は `default-external-encoding-1` で解消済み。**同じ直し方が効くかは下流を確かめてから**(ソース側は読み出しだけでは足りなかった) |
| **Y**([issue](../../issues/argv-encoding-classification.md)) | **引数の分類がロケール依存**。ロケールのエンコーディングで不正なバイトを含む `ARGV` を `Regexp#===` に掛けて落ちる | ファイル名だけでなく `-I` / `-D` / `-l` の値も対象。**ファイルを読む前**に落ちる | **実測**(2026-08-20、master でも再現) | **引き金が X と逆向き** — あちらはロケール無し × ファイルの中身、こちらはロケール有り × 引数のバイト列 |
