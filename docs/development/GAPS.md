...
| **AC**([issue](../../issues/popcount-and-long-bit-scan-builtins.md)) | **`__builtin_popcount` / `-l` / `-ll` が無い**。ビット走査の `l` 綴り(`__builtin_clzl` / `__builtin_ctzl`)も無い | ビットセットを扱う拡張。gcc なら通るコードが通らない。コーパス候補 roaring 0.4.1 の 4 TU 中 3 TU がここで止まる | **実測**(2026-08-26、ホスト gcc 13.3.0 と [run 32880666098](https://github.com/nuna/rubycc/actions/runs/32880666098)) | 既存の `__builtin_ctz` 系(`BuiltinBitScan`)と同じ経路。x86-64 の `popcnt` はベースラインに無い命令なので、出す命令列は着手時の判断 |
