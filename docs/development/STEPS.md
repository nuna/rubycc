...
## m4/aarch64-alloca-bitscan-2 — sp を動かすために、sp を基準にするのをやめる(M4 A4)

aarch64 バックエンド最後のギャップ `:alloca` を実装した。**これで、ジェネレータが
出しうる IR 命令のうち、どちらのバックエンドも拒否するものは無くなった。**

### 問題は alloca そのものではなく、フレームの基準

alloca 自体は 5 命令で終わる — サイズを 16 に切り上げ、sp を下げ、下がった sp を
返すだけである。難しいのは**このバックエンドが sp 基準でフレームを組んでいる**
ことで、これは A2(Step 70)の意図的な設計だった: A64 の ldr/str のスケール付き
即値はベースレジスタから**上方向に 32 KiB** 届くが、符号付き非スケール形は
**下方向に 256 バイト**しか届かない。したがって「フレーム底 + 正のオフセット」が
唯一まともに届く配置で、x86-64 の「rbp − 負のオフセット」は移植できない。

alloca はその底を動かす。**全スロットの番地が一斉に変わる。**

### 採った形 — alloca を含む関数だけ x29 で固定する

```
プロローグ:  sub sp, sp, #frame       エピローグ:  mov sp, x29
             stp x29, x30, [sp, #off]              ldp x29, x30, [x29, #off]
             mov x29, sp   ← alloca 関数のみ       add sp, sp, #frame
                                                   ret
```

プロローグの順序は選択の余地が無い。**呼び出し元の x29 が退避レコードに入る前に
上書きしてはならず**、かつ**パラメータの spill より前に置かねばならない**
(spill のストアがすでにフレーム基底を経由するため)。そのため退避レコードだけは
`base: SP` を明示して書く — その瞬間の x29 は呼び出し元の値だからである。

エピローグの `mov sp, x29` は、**本体で確保した全ブロックを 1 命令で解放する**。
alloca の記憶域が「スコープの終わり」ではなく「関数の終わり」まで生きるという
規約が、そのままこの 1 命令になっている。

フレーム基底は `frame_base_register`(`@uses_alloca ? FP : SP`)1 箇所に集約し、
スロット参照・可変長退避領域・呼び出し元スタック引数・大フレーム用のアドレス合成が
すべてここを問う。**alloca を含まない関数の出力は 1 バイトも変わらない**
(答えが SP のままなので)。

### スタック引数との相互作用 — ここだけは sp 基準のまま

AAPCS64 は callee が「`bl` を実行したときの sp」からスタック引数を読むと定める。
alloca 関数では sp がフレーム底を指していないので、プロローグが確保する
**静的 outgoing 領域は到達不能**になる。そこで:

- alloca 関数は静的 outgoing 領域を**まったく確保しない**(`@outgoing_size = 0`)。
- 各呼び出しが直前に `sub sp, sp, #n` で自分の領域を**確保済みブロックの下に**取り、
  引数を **sp 基準**で書き、復帰後に `add sp, sp, #n` で**返す**。

返すのが重要で、返さないとループ内の呼び出しが 1 周ごとにスタックを 1 領域ずつ
削り取る。この 2 つ(ブロックは積み上がる / 呼び出し領域は積み上がらない)は
**逆方向の間違え方**なので、実行テストで両方を同時に検査している。

### 命令エンコーディング

いずれも ARM DDI 0487 のビットフィールド定義から組み立て、クロス as で実測確認した。

| 用途 | 命令 | 語 | 根拠 |
|---|---|---|---|
| 16 切り上げ | `and Xd, Xn, #-16` | `0x927CEC00` | Logical (immediate)。0xFFFF…FFF0 は 60 個の 1 の連なりなので、ビットマスク即値で表せる(N=1, imms=59 が長さ−1, immr=60 が右回転)。movz/movk 4 命令が 1 命令になる |
| sp を下げる | `sub sp, sp, Xm, uxtx` | `0xCB206000` | Add/subtract (**extended register**)。シフト付きレジスタ形式ではレジスタ番号 31 が**ゼロレジスタ**と読まれるので、sp を名指せるのは拡張レジスタ形式(と即値形式)だけである |
| x29 の設定・復帰 | `mov x29, sp` / `mov sp, x29` | `add Xd, Xn, #0` | 同じ理由で add-immediate 形式。既存の `emit_add_imm` をそのまま使う |

### 先行実装から変えた点

`m4/aarch64-glibc-acceptance`(b4b17da)に同じ設計の実装があり、x29 固定・
per-call の outgoing 領域という骨格は妥当だったのでそのまま採った。変えたのは 3 点。

1. **`-16` のマスクを `materialize(B, -16, 64)` から AND 即値 1 命令に**した。
   先行実装は movz + movk × 3 の 4 命令を使い、スクラッチ B も潰していた。
   alloca 1 回あたり 4 命令が 1 命令になり、B が空く。
2. **静的 outgoing 領域を alloca 関数では確保しないことにした。** 先行実装は
   確保したまま使わない(死んだ領域が残る)。同時に退避レコードがフレーム底に
   来るので、AAPCS64 の通常のフレームレコード配置と一致する。
3. **命名**。`store_sp_at` → `store_outgoing_at`(「なぜ sp 基準なのか」が
   名前から分かる)、`reclaim_dynamic_call_stack` → `release_dynamic_argument_area`。
   また `stack_argument_bytes` を `place_arguments` に畳んだ(1 箇所からしか
   呼ばれず、返り値と対で使うため分離する利点が無い)。

なお先行実装には `emit_ftoi` の変更も混ざっていたが、あれは alloca と無関係な
別件で、master 側では既にジェネレータが実幅を渡す形で解決済みだったため取り込んでいない。

### `UnsupportedError` の扱い

これで raise する箇所がゼロになったので、`unsupported` ヘルパ(死にコード)は
削除した。**例外クラスとドライバの rescue は残した** — 新しいターゲットを書くときの
規約(「降ろせないものは明示的に拒否する。もっともらしい誤りを出さない」)が
これだからで、クラスコメントにその旨を書いた。

`test_driver.rb` の「未対応構文が診断される」テストは、名指しできる構文が
無くなったので**注入方式に変えた**。この 4 年で例のほうが 4 回入れ替わって
いる(文字列リテラル → 間接呼び出し → struct 代入 → alloca)ので、5 つ目を
探すより、ドライバの rescue 節そのものを対象にするほうが素直である。
minitest 6 は mock を同梱しないため、特異メソッドの差し替えと `ensure` での
復帰を手書きした。

### 検証

- エンコーディング: `test_aarch64_backend.rb` に 5 件(切り上げと sp 操作・
  x29 固定・**alloca を含まない関数が不変であること**・per-call の outgoing 領域・
  静的領域を取らないこと)。
- 実行差分(クロス gcc + qemu、`test_aarch64_execution.rb`): 基本(呼び出しを
  跨いだ保持・非重複・16 整列)・**10 引数のスタック引数呼び出し**・**ループ内
  alloca**・**可変長関数内 alloca**(11 引数で退避領域からスタックへ跨ぐ)・
  **9000 long の大フレーム**(スケール付き即値も 12 ビット add 即値も超える)。
  加えて逆アセンブル健全性 1 件(AND ビットマスク即値と拡張レジスタ sub は
  他のどこも出さない語なので、実物の逆アセンブラに通す)。
- サンプル: `examples/m4/aarch64_alloca_bitscan_2_alloca.c`(両機種で差分実行)。
- `AARCH64_PENDING` が空になった(`test_examples.rb` / `test_examples_aarch64.rb`)。

**サンプル作成中に踏んだ実測**: `printf("%ld distinct=%d", f(&flag), flag)` は
**gcc と rubycc で答えが違う**。引数の評価順は未規定(6.5.2.2p10)で、gcc は
`flag` を呼び出しより先に読む。差分テストの失敗として出たが、これはコード生成の
バグではなく**サンプルの書き方の誤り**である。文を分けて解消した。
...
## ci/simplify-skip-baseline-1 — 台帳を厳しくしすぎた分を戻す(CI)

`tools/ci_check_skips.rb` の skip ガードは 4 層になっていた。

1. `max_skips` / `min_runs` — 総数の上下限
2. `allowed_skips` — 各 skip が「テスト名パターン + 正規化済み理由」のルールに
   **ちょうど 1 つ**一致すること、およびルールごとの `min_count` / `max_count`
3. `expected_skips` — skip 総数の完全一致(`CI_ENFORCE_SKIP_BASELINE=1` のとき)
4. `skip_fingerprint` — skip 集合(テスト名 + 正規化理由)の SHA-256 完全一致(同上)

このうち **3 と 4 を削除した**。層 3 は層 4 に完全に包含される(集合が同じなら総数も同じ)。
問題は層 4 の側にある。

### 層 4 の限界効用は狭く、コストは構造的だった

層 4 が層 2 を超えて捕まえるのは、**「承認済みの理由のまま、新しい/改名されたテスト名が
skip し、かつそのルールの `max_count` を超えない」**という場合だけである。
未知の理由も、既知理由の件数増も、既知 skip が別原因に置き換わった場合も、層 2 が捕まえる
(置き換えは総数が変わらないので、層 3 では捕まらない)。

一方でコストは狭くない。fingerprint はテスト名と理由の集合のハッシュなので、
**手元で再計算できない** — ローカル 41 / CI 40 という環境差が実在し、正しい値は
CI ログにしか無い。そして test/ の skip 行に触れるコミットは
**直近 120 件のうち 2 割前後**ある(`skip` を含む差分行を数えた粗い近似で、
数え方により 19〜23 件。正確な値ではなく桁の話として読むこと)。つまりそのたびに
「push → 落ちる → artifact を取得 → 再計算 → push」の往復が入る。
実際に直近 2 回それが起きた。

`m4/aarch64-alloca-bitscan-3` で「skip が減ったときに落ちるのは設計どおり」と書いたのは、
層 1 の目的に照らせば正しい。しかしその判断で守っていたのは層 3+4 であって、
本来の失敗モード(ツールチェーン消失で全部 skip なのに緑)は層 1 が単体で捕まえる。
守っている対象と払っているコストが釣り合っていなかった。

### `expires` も廃止した

`expires` は profile に 1 か月の期限を付け、切れると強制時に落ちる仕組みだった。
再測定が高コストな値(native ランナーの手動 dispatch が要る)に期限を付けると、
**内容と無関係な日付で定期的に故障する**だけになる。現に `native-x86` は 2026-09-11 に
自動で落ちる予定だった。「この値を信用するな」の表明は `provisional` が担うので、
時計は不要である。

### `native-aarch64` の `provisional` は false に戻した

このフラグは前ステップで「古い fingerprint を信用するな」の意味で立てたものである。
その fingerprint ごと消えるので役目が終わった。ただし**native ランナーでの再測定は
依然として行われていない**ので、`note` をその事実を記録する短文に書き換えた
(削除はしない)。残る層 1+2 は上限側の検査なので、実際の skip がベースラインより
少ない現状でも落ちない。

### 単体テストは「失われた検出力」を明示して置き換えた

`test_strict_profile_rejects_a_new_test_with_an_existing_reason` は層 4 の検証だったので
削除し、同じ入力が**通ること**を確認する
`test_enforced_baseline_no_longer_pins_the_exact_skip_set` に置き換えた。
トレードオフを黙って消すのではなく、テストとして残す。
併せて層 2 の検証を厚くした — 強制時にも未承認理由で落ちること、
`max_count` 超過で落ちること、`max_count` ちょうどは通ること。

### 検証

PR #31 の実 CI ログ(Ruby 3.3 / 4.0、3,097 runs / 40 skips)に対し
`CI_SKIP_PROFILE=native-x86 CI_ENFORCE_SKIP_BASELINE=1` が OK になることを確認した。
さらに同ログの改変コピーで、変更前後の差を実測した。

| 改変 | 変更前 | 変更後 |
|---|---|---|
| skip されたテスト 1 件を改名(理由は承認済み) | FAIL(fingerprint) | **OK** |
| skip 1 件を削除(40 → 39) | FAIL(`expected_skips` + fingerprint) | **OK** |
| 未承認理由の skip を 1 件追加(40 → 41) | FAIL | **FAIL**(`unapproved skip` + `max_skips`) |
| 既存 skip の理由だけを未承認のものに差し替え(40 のまま) | FAIL | **FAIL**(`unapproved skip`) |

最後の行が要点である。層 3(総数)では捕まらない「置き換え」は、
層 4 を消した後も層 2 が単独で捕まえる。

### 数値ガードには余裕を戻した(`max_skips` 40 → 45)

層 3 を消したことで、`max_skips` が唯一の数値ガードになった。ところがその値は
`m4/aarch64-alloca-bitscan-3` で `expected_skips` に合わせて 42 → 40 に下げてあり、
**CI 実測ちょうどで余裕がゼロ**だった。ツール冒頭のコメントは「小さな運用余裕を
持たせている」と述べており、同じコメントが「ローカルでは 41 件」と認めていた —
記述と実態が矛盾し、実際ローカルでは常に落ちる状態だった。

`expected_skips` が消えた以上、`max_skips` は完全一致の代役ではない。45 に戻した。

**緩めても検出力は落ちない。** 原因ごとの精密な検出は層 2 の `max_count` が担う
(例: pkg-config 欠落は `TestPkgconf` ルールの `max_count: 1` で捕まる)。
層 1 の役目は「ツールチェーンが丸ごと消えて数十〜数百件が skip 化した」という
壊滅的な取りこぼしの検出だけであり、そこに数件の余裕は影響しない。
実ログの改変コピーで、上の表の 3 ケースが 45 でも同じ結果になることを確認した。

これは一般則として書いておく価値がある。**閾値を実測値ちょうどに詰めると、
その閾値は「異常の検出器」ではなく「環境差の検出器」になる。**

---

## corpus-sqlite3-pg-1 — sqlite3 は「どちらの経路が建ったか」が記録の中身(M5 H4)

v1.0 に残っていた唯一の定量ゲートは R10(コーパスの 90% が gem 自身のテストに合格)で、
**29/34 = 85.3%**、90% に必要な 31 件まで 2 件足りなかった。分母に居て記録が無いのは
`openssl` / `rbs` / `oj` / `sqlite3` / `pg` の 5 件。このうち sqlite3 と pg は
**ビルドと require までは Step 193・202 で実測済み**で、上流スイートの記録だけが無い。
そこで安い方から片付けた。

### 走らせていなかった理由はレシピではなく環境だった

sqlite3 のレシピは、この gem を分母に入れた時点で書かれていたのに一度も走っていなかった。
ホストに `libsqlite3-dev` が無かったためである。R10 プロファイル
`--enable-system-libraries` は**システムの libsqlite3 を使う**という意味なので、
開発パッケージが無ければそもそも成立しない。**「未検証」の理由が rubycc 側にあるとは
限らない**という、GAPS §3 と同じ形がここにもあった。

### 記録の中身は件数ではなく経路

334 runs / 661 assertions / 0 failures / 0 errors / 5 skips。
ただしこの gem で意味を持つのは件数の方ではない。既定の経路は mini_portile2 + configure で
**R10 が明示的に除外している形**であり、分母に入れられるのは
`--enable-system-libraries` を明示したときだけである。したがって sanity 式

```
injected_so_loaded? && SQLite3::SQLITE_PACKAGED_LIBRARIES == false
```

が記録の本体で、「テストが通った」はその上に乗る。この定数は
`ext/sqlite3/sqlite3.c:215-217` が**同梱ライブラリを使ったときだけ true** にするので、
経路の観測点として過不足がない。

### skip 5 件のうち 3 件は、選んだプロファイルそのものが理由

`test/test_database_uri.rb:9,20,34` は
`skip("system libraries may not allow URIs") unless SQLite3::SQLITE_PACKAGED_LIBRARIES`。
**R10 に適合する経路を選ぶと、上流が自分でこの 3 件を落とす。** rubycc とは無関係で、
gcc で建てても同じ 3 件が飛ぶ。この「プロファイルの代償」は `notes` に手で書いた —
件数だけを残すと、次に読む人が rubycc の欠落と読み違える。

R10 通過率 **29/34 = 85.3% → 30/34 = 88.2%**(90% には 31 件、**あと 1 件**)。

---

## corpus-sqlite3-pg-2 — 合否の率を比べる前に、量そのものを測る(M5 H4)

pg 1.6.3 の上流 spec は `1040 examples` で、1 回目は **0 failures**、2 回目は
**2 failures** だった。どちらも壁時計に依存する spec である。
**「緑の回を選ぶ」は証拠にならない**(`corpus-ninety-2` で puma を回したときの規律)ので、
gcc 対照を建てて突き合わせた。

### 率で比べたら、rubycc の欠陥に見えた

失敗が出た 3 ファイルに絞り、シードを固定せず**両側 10 回ずつ**回した:
...
4 倍差である。libpq はシステムの共有ライブラリで両側共通、違うのは
rubycc がビルドした `pg_ext.so` だけ — **接続のポーリングとタイムアウト計算を持つ層**。
この時点では rubycc 側の欠陥に見えていた。

### 量を測ったら、逆の答えが出た

合否は「壁時計の値がしきい値の内側か」の 1 ビットでしかない。**しきい値ではなく
所要時間そのもの**を、rspec を挟まずに測った(`/tmp/pg_connect_timing.rb`:
listen だけして accept しないソケットへ 2 ホスト指定・`connect_timeout: 1` で接続し、
**単調時計と壁時計の両方**を記録する)。両側 40 回:

| | `mono` の範囲 | 1.9 秒未満 | `wall` が `mono` と 0.05 秒以上食い違った回 |
|---|---|---|---|
| rubycc | **2.003〜2.010** | **0/40** | 2/40 |
| gcc 対照 | **2.002〜2.020** | **0/40** | 3/40 |

**接続の所要時間は両側で区別がつかない。** 食い違っていたのは `Time.now` の側で、
壁時計が **約 1.2 秒巻き戻っていた**。原因はホストにある — WSL2 の時計がずれ、
`systemd-timesyncd` が step で補正している。`journalctl` 自身のログが
巻き戻りを記録していた:

```
Aug 11 11:34:12  Timed out waiting for reply from 185.125.190.58:123 (ntp.ubuntu.com).
Aug 11 11:34:10  Contacted time server 91.189.91.157:123 (ntp.ubuntu.com).
```

8/10 対 2/10 は、**1 回あたり 5〜7% の事象が、壁時計判定を十数個持つスイートに
当たったときのばらつき**だった。**率の比較は、同じ壊れやすい量に依存する検査が
多数あるスイートでは分解能が足りない。**

### 原因を取り除いてから測り直した

`systemd-timesyncd` を止めた状態で全スイートを 3 回連続で回し、
**シードが毎回違う(5811 / 18434 / 30697)まま 1040 examples / 0 failures** を確認してから
記録した。**「緑の回を選ぶ」のではなく「緑にならない原因を外してから測る」**なら、
再現性のある証拠になる。

### 先に踏んだ罠 — 作業ディレクトリのパス長

最初の実行は example が 1 件も走らずに終わった。上流 spec は自前で PostgreSQL を起動するが、
その UNIX ドメインソケットのパスが **107 バイト上限**(`sun_path`)を超えていた:

```
Unix-domain socket path ".../tmp_test_specs/.s.PGSQL.23456" is too long (maximum 107 bytes)
```

`VERIFY_WORK` を短いパスに変えるだけで解消する。**ツールの既定
(`<tmpdir>/rubycc_verify_gem_tests`)は短いので既定のままなら踏まない** —
こちらが長いパスを渡したせいである。同じ理由で pending が 2 件出る
(ホストで既定の PostgreSQL が 5432 を掴んでいると、上流自身が `skip` する)。
どちらも `notes` に書いた。

### ついでに測った rbs — 分母は小さくしない

90% への 2 件目の候補として `rbs` 3.10.0 も回した。**FAIL**
(707 tests / 5,414 assertions / 18 failures / 7 errors / 10 omissions)。
落ちるのは `RDocPluginParserTest` で、原因は C 拡張ではなく上流の純 Ruby 側 —
ホスト Ruby 3.4 同梱の `RDoc::TokenStream#collect_tokens` と引数が合っていない。
**gcc 対照も桁まで同じ数字**だったので、`byebug` / `unicorn` / `debug` に適用した
除外基準を満たす。

**それでも分母から外さなかった。** pg が通った時点で 90% を超えており、
**分母を小さくして達成するのは、達成の意味を薄める**。外すかどうかは別の判断として
ROADMAP に残した。

R10 通過率 **30/34 = 88.2% → 31/34 = 91.2%**。**DESIGN R10 の 90% ラインに、
pg / sqlite3 を含む今の分母で到達した。**

---

## gaps-s-t-u-1 — libc のバージョンは、libc に訊く(GAPS U 解消)

同梱 `features.h` が `__GLIBC_MINOR__ 39` を**ハードコード**していた。単一のヘッダ一式を
全ホストに配る設計(R8)の帰結だが、**壊れ方が過大申告の向き**である点が問題だった:
glibc 2.34 のホスト(RHEL 9 / Amazon Linux 2023)で `#if __GLIBC_PREREQ(2, 38)` の
**新しい枝**が選ばれ、その枝が 2.38 で入ったシンボルを使うと**リンクや dlopen で
undefined symbol** になる。gcc なら選ばれない枝である。

### どこに訊くのが正しいか

3 つの候補があった。**ホストの `/usr/include`** は distroless に無い(同梱ヘッダが
存在する理由そのもの)。**ホストの gcc** は「gcc 無しで動く」という前提に反する。
残るのは **C ライブラリ自身**で、これは実際にリンクされる相手でもある。

glibc は自分が満たせるインタフェースを `.gnu.version_d`(版定義表)に
`GLIBC_2.2.5` … `GLIBC_2.39` として持っている。**その最大値が答え**である。
`ELFReader` に版定義の読み出しを足し(gABI の仕様から実装。Verdef / Verdaux は
配列ではなく**各レコードが次への距離を持つ連結リスト**なので、リンクを辿りつつ
`sh_info` の件数で歩数を縛る)、`Preprocess::GlibcVersion` が
`LibraryResolver` 経由で `-lc` と同じ解決を行って読む。

**ホスト固有のパスは 1 文字も書いていない。** target を渡せば target の libc を測る
(`test-ci-implementation-9` で「探索パスは target に従う」と決めた線に乗せた)。

### 測れないときは、黙って 0 にしない

musl・libc が見つからないクロス環境・読めない ELF では **nil**(測定不能)を返し、
マクロを**定義しない**。同梱 `features.h` 側は `#ifndef` ガードにして
従来値 39 をフォールバックとして残したので、**測定が届かない経路の挙動は
これまでと同一**である。musl 対象が変わらないことはテストで固定した。

**既知の過小申告**も記録しておく: 新しいシンボルを 1 つも足さなかった glibc リリース
(2.19・2.20・2.21・2.37)は版ノードを持たないので、そのホストでは 1 つ前の版を答える。
**安全な向きの誤差**(古い枝を選ぶだけで、無いシンボルには手を出さない)なので、
バナー文字列を正規表現で読むより構造を読む方を採った。

### 受け入れ

`test_header_abi.rb` の `__GLIBC_MINOR__` / `__GLIBC_PREREQ` 検査は**ホストの gcc との
差分検査**である。これが**どのホストでも通るようになった**ことが、この作業の受け入れである
(従来は 2.39 のホストでしか通らなかった)。このホストでの実測は 39 で、
`ldd` と gcc の申告に一致した。`glibc_minor: 34` を渡した場合に
`__GLIBC_PREREQ(2, 38)` に 34 が入る(= 偽になる)ことも固定した。

**採番の衝突も直した**: 解消済みの「システム include 探索パスが x86-64 決め打ち」を
当初 U と採番していたが、開いている U と衝突していたので**閉じた側を V** に振り直した。

---

## gaps-s-t-u-2 — 溢れた場所ではなく、決められなかった場所を指す(GAPS T の診断)

`pt b[] = { {1,2}, fp(), {5,6} };`(`fp()` は `pt` を返す)は gcc が通し、
rubycc は拒否する。**この限界は今回そのまま残す**(解消にはパーサに型を引く手段が要る)。
直したのは**報告の仕方**である。従来はこう言っていた:
...
**2 つとも誤誘導だった。** 指している `{5,6}` は完全に正しい要素であり、
「scalar」に至っては、そこにスカラは登場しない。

### 溢れは原因ではなく結果である

型フックが無い経路(パーサが宣言の型を完成させる場面)では、struct の部分オブジェクトに
立っている式が「その struct 全体を初期化する単一式」なのか「波括弧を省略した並びの先頭」
なのかを**区別できない**。resolver は後者と仮定してメンバへ降り、続く項目を吸い込み、
**リストの末尾で溢れる**。報告されていたのはこの溢れ — 原因から 2 つ隔たった場所である。

そこで、型を決められなかった式を記録しておき、溢れの診断が出るときは**その式**を指す
ようにした。現在の出力:
...
GAPS T は「**配列の要素数をパーサが数える文脈で**」と書いていたが、**明示長でも同じく落ちる**
(`pt b[3] = { ... }`)。パーサは長さ推論のためだけでなく、**形の検査**のためにも型無しで
解決するからである。GAPS の記述をこの実測に合わせた。

ついでに gcc 側の挙動も測った。**ファイルスコープでは gcc も拒否する**
(`initializer element is not constant`)ので、この差が出るのはブロックスコープに限る。

### 本来の診断は残した

`excess elements in initializer` / `excess elements in scalar initializer` は、
**本当に要素が余っているとき**の正しい報告なので、そのまま残っている
(`int a[2] = {1,2,3}` / `int x = {1,2}`)。新しい文言に置き換わるのは、
**その実行が型を決められない式に出会っていたときだけ**である。両方をテストで固定した。

---

## gaps-s-t-u-3 — `long double` は直さず、書く(GAPS S の文書化)

`long double` を 8 バイトの `double` として扱っている件(DESIGN 3.3 の既知の制限)は、
**v1.0 では挙動を変えず、README と CHANGELOG に明記するに留めた**(ユーザ判断、2026-08-11)。

### なぜ今は直さないか、なぜ診断エラーにもしないか

本修正は x86-64 の x87 80 ビットと aarch64 の IEEE binary128 の**両方**が要り、
パーサ・定数畳み込み・ABI 分類・`va_arg` に及ぶ。1 ステップではない。

一方、「可変長引数に `long double` を渡したら診断エラー」にする案は、不変条件
(未対応機能は黙って壊さない)には適う。**それでも採らなかった**のは、実害が
**測定済みで oj の 1 テストに限定されている**のに対し、いまエラーにすると
**今ビルドできている gem がビルドできなくなる**からである。副作用の方が広い。

**代わりに、黙っているのをやめた。** README の既知の制限に、何が起きるかを
機序ごと書いた — 演算は double の精度になること、`sizeof` が 8 であること、
`printf("%Lg", x)` が壊れるのは glibc が 16 バイト読むのに rubycc が 8 バイトしか
積まないからであること、そしてそれが oj のスイートで**対照との唯一の差**として
実測されていること。CHANGELOG にも同じ趣旨を 1 行で入れた。

### 「早期に対応する」を計画として残した

先送りの記録は、期限を持たないと腐る。ROADMAP §3 の負債表と §3.1 の割り当て表に
**v1.0 直後に最優先で着手**と書き、着手時の段取りも 2 段階で残した:
**(1) 可変長引数に渡すときだけ double を 80 ビット / binary128 に変換して積む**
(double は両形式の部分集合なので変換は無損失。観測されている実害はこれで閉じるが、
`sizeof` の食い違いは残る)、**(2) 演算そのもの**。

§3.1 の当初の見立て(「feature-gated ゆえ着手するまで観測不能」)は**外れていた**ので、
その旨も明記して置き換えた。H4 で oj が実害を出した時点で、見立ての方が誤りだった。
...
M4 の最後の未測定項目「aarch64 musl の全受入れ」に着手した。これまでの障害は
**`aarch64-linux-musl` のクロスツールチェーンが存在しない**ことだったが、
**arm64 の Alpine コンテナの中では musl の gcc がネイティブ**なので、障害ごと消える。
glibc 側で同じ発想が効いた(`glibc-minor-host-arch-2`)直後に、musl でも同じ手が使えた。

結果は **3,109 runs / 8 failures / 6 errors / 753 skips**。分類すると:

| 分類 | 件数 |
|---|---|
| **rubycc の欠陥** | **1** |
| ギャップ W(gcc 14 が警告をエラーに格上げ) | 5 |
| 自分のテストのホスト前提 | 1 |
| エミュレーションの壁時計(`TestDosResilience`) | 3 |
| コンテナ環境(mkmf の開発ツール不足ほか) | 4 |

### 唯一の欠陥は、予告されていた場所だった

```
gcc(musl/aarch64): FP_ILOGB0 = -2147483648   FP_ILOGBNAN = -2147483648
rubycc            : FP_ILOGB0 = -2147483647   FP_ILOGBNAN =  2147483647
```

同梱 `math.h` は **`__aarch64__` だけで分岐**していた。ところが
**musl は機種に依らず両方 INT_MIN** で、機種で分かれるのは glibc の方である。
libc も選択に入れる形に直した。
...
これで「freestanding / 共通層は機種に依らない」系の思い込みは
`float.h`(Step 201)・`math.h`(`test-ci-implementation-2`)・include 探索パス
(ギャップ V)に続いて **4 件目**になる。今回はその変種で、
**「機種で分岐すれば足りる」(libc を忘れる)**という形だった。

### 自分のテストが 2 度目のホスト前提を踏んだ

`test_glibc_minor_can_be_pinned_and_reaches_the_version_gate` が musl ホストで落ちた。
glibc のバージョンマクロは **glibc ターゲットのときだけ定義する**仕様なので、
musl ホストでは既定の `libc` が musl になり、固定した値が使われない。
テストが `libc:` を名指ししていなかった。`glibc-minor-host-arch-1` と同じ形の誤りで、
**同じ週に 2 度**踏んだことになる — 新しいテストを書くときは、
**そのテストがホストの何を仮定しているかを先に言葉にする**。

---

## m4-aarch64-acceptance-3 — 対照が拒否することにも意味がある(ギャップ W 解消)

gcc 14 のホスト(Alpine / Debian trixie)で、**対照側の gcc がコンパイルに失敗**して
差分テストが 5 件エラーになっていた。rubycc の正否と無関係に落ちるので、
gcc が上がった時点で CI が壊れる。**3 種類に分かれた**のが、この件の要点である。

### (1) 言語版を固定していなかった

対照 gcc を `-std` 無しで起動していたので、**コンパイラ既定の言語版**に従っていた
(gcc 13 = gnu17、gcc 14 = gnu23)。rubycc が実装しているのは C11/C17 サブセット +
GNU 拡張なので、`-std=gnu17` を明示した。値は 1 か所の定数に置き、
3 つのヘルパと `test_examples.rb` がそれを参照する
(「両側のビルド設定は 1 か所から取る」— ROADMAP §2)。
**リンク専用の起動には付けていない**。

### (2) テストのソースが本当に誤っていた

`TestAtomicType` の差分ソースが
...
を含んでいた。`_Atomic int *`(atomic な int へのポインタ)と
`int * _Atomic`(int への atomic なポインタ)は**別の型**なので、
**gcc の言い分が正しい**。gcc 13 が警告で見逃していただけである。ソースを直した。

### (3) 意図的に受理している旧構文は、対照に飲ませる必要がある

K&R の 2 件(テストとサンプル)は implicit int を使う。これは C99 が削除した構文で、
**rubycc は K&R 互換のために意図的に受理している**(Step `atomic-type-10`。
io-console などが今も出す)。対照が拒否すると**比較そのものが成立しない**ので、
この 2 件にだけ `-fpermissive` をオプトインで渡す。

**全体には適用しない。** 他のテストでは「gcc がコンパイルを拒否すること」自体が
**テスト側の C が壊れている合図**であり、実際 (2) はその形で見つかった。
緩和を全体に効かせていたら、(2) は今も警告のまま隠れていた。
...
## docs-audience-split-2 — 完結した記録を 1 つに畳み、作業課題の置き場を決めた(ドキュメント)

分割の直後に「**課題も作業もすべて終わっていて、今後更新される見込みの無い文書はあるか**」を
調べた。15 文書のうち該当は **1 件だけ**だった。

### `VARIADIC-SCAN.md` を STEPS に畳んだ

内容は `tools/scan_corpus_variadics.rb` による **2026-08-09 時点の c-testsuite 走査
1 回分**の記録だった。実測値は次のとおりである(走査ファイル 220):

| 項目 | 件数 |
|---|---:|
| 構造体・union の `va_arg` 候補 | 14 |
| 構造体・union の variadic caller 候補 | 142 |
| variadic function pointer 候補 | 1 |
| 合計 finding | 157 |

`va_arg` 候補 14 件はすべて `single-exec/00204.c` の HFA と小構造体で、struct `va_arg` を
診断拒否する現行仕様と一致する。caller 候補の大半は `printf` 宣言に対するヒューリスティックで、
**スキャナの結果だけでは「実際に struct が渡される」とは分類しない**。再現は

```sh
ruby tools/scan_corpus_variadics.rb --root test/external/c-testsuite --format json
```

**この文書が答えようとしていた問いは、`test-ci-implementation-7` がより強い証拠で
決着させていた** — rubycc は struct の可変長引数渡しと `va_arg(struct)` を
**ハードエラーで拒否する**ので、ビルドが通っている R10 対象はその時点で不使用が
証明されている。字句スキャンは不在証明にならないが、コンパイラの拒否は証明になる。

コーパス側の同種スキャンは `R10-CORPUS-SCAN.md`(生成物)が継続して担当する。
スキャナ本体とそのテストは残るので、数値は再取得できる。**参照はゼロだった**ので、
記録をここへ畳んで文書は消した。

### 「近いが凍結できない」ものの区別

同時に調べた 4 件は、完結しているように見えて更新される:
`BENCHMARKS.md` / `THROUGHPUT.md` は **M6 のレジスタ割付で必ず更新される回帰の基準値**、
`R10-CORPUS-SCAN.md` / `R10-MANUAL-CLASSIFICATION.md` は**生成物**、
`TEST-PLAN.md` は未着手項目が実在(2B-1・2B-3・R10-4〜R10-7)、
`security-dos-review.md` は DoS 上限の再調整が H4 に残っている。
`RELEASE-CHECKLIST.md` はタグ push と `gem push` の 2 行が未実施で、**リリース後に凍結候補**になる。

### 作業課題は `issues/` に置くことにした

これまで、未消化の作業は **4 か所に散っていた** — `GAPS.md`(未解消ギャップ)、
`ROADMAP.md`(計画)、`TEST-PLAN.md`(テストの未着手項目)、
`security-dos-review.md`(今後の課題)。どれも「文書の主題のついで」に課題を抱えており、
**状態(未着手/着手中/完了)を持たない**ので、完了しても消し込まれずに残る。
実際、この 2 日で `ROADMAP.md` の現在地と `RELEASE-CHECKLIST.md` の 3 か所が
その形で古くなっていた。

**1 PR に収まる粒度**で課題を 1 ファイルにし、課題内容・作業ログ・状態を
構造化して持たせる。置き場と規約は [`issues/README.md`](../../issues/README.md)。

設計上の判断は 4 つ。

- **ファイル名は着手するブランチ名**にする。**連番は使わない** — 並行作業で 2 回衝突した
  実績があり、ステップ ID の採番方式を変えた理由でもある。ブランチ名なら
  課題・ブランチ・ステップ ID・PR が 1 本の線で繋がる
- **状態を front matter に持たせる**。「終わったのに open のまま」を防ぐのが状態欄の
  全目的で、散文に埋めていたときはそれを検査する手段が無かった。
  `test/test_issue_docs.rb` が語彙と整合(`done` なら `pr` と `closed` がある等)を検査する
- **索引ファイルを作らない**。`grep -l "^status: open"` で足りる。索引は更新漏れの分だけ損で、
  今回まさに「現在値が 3 か所に書き写されて 2 か所が古い」を直したところである
- **設計判断は STEPS に置く**。issue の「決着」節は STEPS を指すだけにして、二重管理を作らない

### 散っていた未消化項目を 5 件移した

移設元には**リンクだけ**を残し、本文は複製していない。

| issue | 移設元 |
|---|---|
| `long-double-varargs` | `GAPS.md` のギャップ S |
| `acceptance-fixture-offline` | `TEST-PLAN.md` 2B-1 |
| `acceptance-fixture-required-job` | `TEST-PLAN.md` 2B-3 |
| `r10-verification-completeness` | `TEST-PLAN.md` R10-4〜R10-7 |
| `dos-limits-retune` | `security-dos-review.md` の「今後の課題」 |
| `release-v1-0-0` | `RELEASE-CHECKLIST.md` §5 の未実施 2 行 |

移設の途中で**規約の穴が 1 つ見つかった**。リリース操作(タグ push・`gem push`)は
**リポジトリを変更しないので PR を持たない**。`done` に PR 番号を要求すると、
そこで嘘の番号を書くことになる。`pr: none` を許し、決着節に何をもって完了としたかを
書く形にした。**規約は、最初の実物 6 件を通すまで正しさが分からない。**
...
## long-double-varargs-1 — 幅を変えずに、渡すときだけ本物の形式へ(GAPS S の第 1 段)

`long double` を可変長引数に渡すと値が壊れていた。glibc の `printf` は `%Lg` で
16 バイト(80 ビット拡張)を読むのに、rubycc は 8 バイトしか積んでいなかった。

**第 1 段の設計は「幅を変えずに、渡すときだけ変換する」**である。`sizeof(long double)` を
16 にするのはオブジェクトファイルの ABI を動かすので、他の既知逸脱とまとめて major で
閉じる([platform-abi-alignment](../../issues/platform-abi-alignment.md))。

### 型の識別を取り戻すのが前提だった

着手前に測ったところ、`long double` は**パース時点で `Type::Double` に潰されていた**ので、
呼び出し側には「これは long double だった」と知る手段が無かった。
`Type::LongDouble`(`FloatType.new("long double", 8)`)を足して識別を取り戻し、
**サイズは 8 のまま**にした。`Type::Double` を直接比較している箇所は lib 全体で 9 か所しか
なく、`float?` とサイズ 8 の性質は変わらないので、波及は小さい。

### 私の指示が、部分的に正しいという最悪の形の欠陥を作った

移譲の指示に「**演算は double と同一**」と書いた。意図は「精度と IR を変えない」だったが、
実装は文字どおり**型名も落とした** — 通常の算術変換が `Type::Double` を返すようにした。
結果:

```c
long double a = 1.25L, b = 1.0L;
printf("%Lg %Lg\n", a, a + b);   /* rubycc: 1.25 -1.25 / gcc: 1.25 2.25 */
```

**変数は正しく、式は壊れる。** 部分的に正しいので気づきにくく、defect としては最悪の形である。
C11 6.3.1.8 は「一方が `long double` なら結果は `long double`」と定めているので、規格どおりに
直した(IR は幅 8 のままなので**生成コードは変わらない** — 名前が残るだけである)。

**教訓**: 「A は B と同じに扱え」という指示は、**どの層で同じなのか**を書かないと通じない。
精度・IR・型名は別の層である。

### 実装が既存バグを 2 件掘り当てた

- **`convert_floating` の `:ftof`**: 幅の判定に**ソース幅**を使っていたため、同幅の
  float→float 変換(`(long double)d`)が「double→float の縮小」と解釈されて値が壊れた
- **aarch64 の `outgoing_argument_bytes` が `:pad_stack` を数えていなかった**:
  `place_arguments` 側は数えるので、**引数領域が足りず他人のフレームを踏む**。
  修正前は qemu で **SIGSEGV** を実測。`__int128` の 16 バイト整列でも既に踏み得た
  **潜在的なメモリ破壊**であり、今回の作業とは独立した収穫である

### 変換はビット操作で、特殊値を名指しで扱う

x87 命令のエンコーダは新設していない。double は 80 ビット拡張・binary128 の**厳密な
部分集合**(指数 11→15 ビット、仮数 53→64/113 ビット)なので、丸めは起きずビット再構成で
足りる。±0・±inf・NaN・非正規数はそれぞれ専用の腕を持つ — 非正規数は広い形式では
正規数になるので `:bit_scan` で先頭 1 を寄せて指数を下げる。**signalling NaN の quiet 化**は
IEEE 754 6.2 の規定であり、gcc の実測とも一致した。x87 の 10〜15 バイト目は psABI 未規定で
gcc はスタックのゴミを積むが、**rubycc はゼロを書く**(N4 の決定性のため)。

### ABI の確認は「gcc が読み戻したバイト列」で取った

「gcc がこう積むはず」ではなく、**gcc 製の callee に `va_arg` で読み戻させ、生バイトを
照合**している(x86-64 で 10 バイト、aarch64 で 16 バイト。NaN ペイロードと quiet ビットまで
一致)。配置も実測した — x86-64 は X87/X87UP のメモリ級で 16 バイト整列、aarch64 は
**可変部でも名前付きと同じ NSRN 規則**で q レジスタを 1 本使う(Apple 変種とは違う)。

### 受け入れ

`oj` の失敗テスト名の集合が **gcc 対照と完全に一致**した(個別実行・シード 1234 で
両側とも 687 runs / 1 failure / 2 errors、名前も同一)。**`UsualTest#test_decimal` は
両側で通り**、`BigDecimal` への変換が `-nan` から `0.123457e1` になった。
`atomic-type-12` 以来「対照と食い違う唯一のテスト」だった件が閉じたことになる。

**測り方の注意**: oj のテストは Oj のグローバル設定を共有するので、**1 プロセスにまとめて
走らせるか個別に走らせるかで失敗件数が変わる**(まとめると rubycc 32 件 / 対照 49 件、
個別だと両側 3 件)。だから件数ではなく**名前の集合**で、かつ**両側を同じ方法で**比べる。

## spill-traffic-cleanup-1 — 生成器とバックエンドの間に、局所変換の層を置いた

レジスタ割付([register-allocation](../../issues/register-allocation.md))に着手する前に
測ったところ、**割付より先にやるべきことがあった**。rubycc は gcc **-O0** に対しても
1.07〜1.97 倍遅い。両者とも spill-everything(全中間値をスタックに materialize する)
なのだから、**この差は割付の有無では説明できない**。

`b[i] += scale * a[i]` を単独でコンパイルすると **gcc -O0 が 41 命令、rubycc が 76 命令**で、
差は 3 つ。**添字 × 4** が定数のスロット往復 + `imul` で 5 命令(gcc は `lea` 1 命令)、
`mov %rax,-0x50(%rbp)` の**次の命令**が `mov -0x50(%rbp),%rax`、定数の materialize が 3 命令。
いずれも**生存区間解析を要しない**。**割付を入れてもこの無駄は残る** — 割付はスロットを
レジスタに置き換えるが、**そもそも不要な store/load を消しはしない**。

だから issue を 2 段に分けた。**本ステップは第 1 段の前半にあたる IR 層**で、
バックエンド側(直後の読み戻し省略・スロット直接参照・単一使用値のストア省略)は
`spill-traffic-cleanup-2` に置く。**速度の実測は両方が揃ってからまとめて取る** —
片方だけの数字は、もう片方が入った後の姿を説明しないからである。

### 層をどこに置いたか

`IR::Simplify` を**生成器とバックエンドの間**に置いた(`compiler.rb` の
`backend.compile(IR::Simplify.run(ir_func))`)。生成器の中に入れると出力が「素の
lowering」でなくなって読みにくくなり、バックエンドの中に入れると 2 つのターゲットに
同じものを 2 度書くことになる。**両バックエンドがこのパスの出力を下ろせる**ので、
継ぎ目にターゲット別の条件は要らない。

### CFG も生存区間も作らずに、健全性を条件で担保する

3 つの変換はどれも**平坦な命令列に対する読み書き回数の集計と、隣の命令**だけで決まる。

- **単一使用コピーの前送り**: `T = <式>; V = T` を `V = <式>` にする。代入は必ずこの形を
  通る。**生成側が V を読む場合(`i = i + 1`)も正しい** — どのバックエンドも
  オペランドをレジスタに読んでから結果を書くからである
- **添字融合**: `:mul`(定数 1/2/4/8)+ **直後**の `:add` を新命令 **`:scaled_add`** に畳む。
  間に何も無いことが安全性の根拠そのもので、`:add` のどちらのオペランドも
  `:mul` が読んでから書き換わっていない。**両ターゲットが 1 命令を持つから IR 命令にした**
  (x86-64 は SIB スケール付き `lea`、AArch64 はシフト付きレジスタの `add`)
- **死結果の除去**: 白リスト(`PURE_OPS`)方式。**`:div`/`:mod` は 0 除算が trap し、
  `:load` は wild pointer が fault する**ので、結果が読まれなくても残す。
  融合で読み手を失った定数はここで落ち、落ちると別の命令が死ぬので**不動点まで回す**

### 守ったのは 2 つ

- **fail-safe**: 読み取り箇所を 1 か所でも漏らすと**生きている値を消してしまう**。
  だから `operand_vregs` はオペランドの形を推論せず**列挙**し、
  **知らない op が 1 つでもあれば関数まるごと素通しする**
- **N4(決定的ビルド)**: 命令を落としても **`vreg_count` を触らない**。
  スロット番号が動くとフレームレイアウトが変わり、同じ入力が同じバイトを出さなくなる

`:scaled_add` の `size` が**オペランド幅ではなくスケール**を表す点は
`docs/internals/IR.md` に明記した(計算は結果がポインタなので常に 64 bit)。
`:load`/`:store` の `size` がアクセス幅を表すのと同じ読みである。

### 差分テストでは見えないものを、バイト列で押さえる

このステップの変換は**命令を減らす**だけなので、gcc 差分テストからは何も見えない
(遅いままでも答えは合う)。そこで `test_ir_simplify.rb` で**変換が起きること**と
**起きてはいけない場合に起きないこと**(2 の冪でないストライド・積を 2 回読む形・
アドレスを取られた「定数」)を押さえ、両バックエンドのテストで `:scaled_add` が
**1 命令に下りるバイト列**を Intel SDM / ARM ARM の符号化から書き下して固定した。

## spill-traffic-cleanup-2 — 書いた直後に読み戻すのをやめた(N2 の到達)

第 1 段(`spill-traffic-cleanup-1`)で IR 層を片付けた続き。こちらは**バックエンド層**で、
`mov %rax,-0x50(%rbp)` の**次の命令**が `mov -0x50(%rbp),%rax` になる、という一番大きな
無駄を取る。

### 「1 バイトも出していない間だけ信じる」

どのスクラッチレジスタがどのスロットの値を持っているかを、関数の生成中だけ追う
(`Backend::SlotResidency`)。**正確な安全条件を作ろうとすると、この層が避けたはずの
生存区間解析とエイリアス解析が要る**。そこで条件を粗いものに倒した:

> **記録してから 1 バイトも emit されていない間だけ、その residency を信じる。**

レジスタもメモリも命令なしには変わらず、全命令が単一の emit を通るので、
**「バッファが伸びていない」= 「レジスタにもメモリにも何も起きていない」**が成り立つ。

この形にした利得は、**Step 11 が記録したエイリアシングが非問題になる**ことである。
アドレスを取ったスロットへポインタ経由でストアすると任意のスロットが変わり得るが、
**そのストア自身のバイトが進行中の residency をすべて終わらせる**。呼び出しも
アトミックもインラインのスタック操作も、1 つも列挙せずに同じ理屈で覆える。

**emit せずに機械状態が変わる例外は 1 つだけ** — IR の `:label`(分岐の合流点)で、
そこには「別の経路で来たレジスタ」が届く。ラベルはバイトを出さないのでバッファ長の
検査では見えず、**両バックエンドが手動で全 residency を捨てる**。
このモジュールが利用側に課す正しさの負担は、**この 1 点だけ**である。

### スロットに書かない値を、IR 層に数えさせた

「書いた命令の**次**の命令だけが読む」値は、**そもそもスロットに書かなくてよい**
(`IR::Simplify#transient_vregs`)。式の中間値はすべてこの形で、`b[i] += scale * a[i]` の
ループでは **12 個のストアのうち 9 個**がこれである。

判定は 4 条件で、どれも平坦な命令列から決まる。書きが 1 回・読みが 1 回であること、
読み手が**直後**の命令であること(この IR の分岐先は必ず `:label` で、`:label` は
読み手にならないから、両者の間に制御は入れない)、パラメータスロットでないこと
(プロローグが命令の前に書く)、そして**生成側と読み手のレジスタファイルと幅が一致**
すること(ベクタレジスタに残した double は、汎用レジスタで読む命令の役に立たない)。

生成側・読み手として認めるのは白リストで、**`:call` はどちらでもない** —
引数のステージングが、値の待っているレジスタをそのまま踏む。

### オペランドをスロットから直接読む(x86-64)

x86-64 は `01 /r`(`add r/m32, r32`)と `03 /r`(`add r32, r/m32`)のように**同じ演算の
両方向**を持つので、第 2 オペランドは ecx を経由せずスロットを直接指せる。
可換な演算(`add`/`imul`/`and`/`or`/`xor`/`eq`/`ne`)は**レジスタに残っている方を第 1
オペランドに回す**と move すら消える。`sub` とシフトは入れ替えられない
(シフト量は cl に届く必要がある)。**スロットに書かれていない値(transient)は
メモリオペランドで名指せない**ので、そちらは必ずレジスタ側に来る。

### 決定性(N4)を壊さないための小さな決め

同じスロットを複数のレジスタが持つことがある(同じ値を 2 回渡す呼び出し)。
どれを使ってもよいが、**最初に見つかったものを選ぶ**と決めた。答えが命令列だけの
関数になり、同じ入力が同じバイトを出す。

### 測った — N2 が 5 カーネルすべてで成立した

`benchmark/run.rb` の C カーネル 5 件、変更前ツリーの worktree と交互のペア計測
(単調時計・5 回中央値、詳細は [BENCHMARKS.md](BENCHMARKS.md)):

| | 変更前 | 変更後 |
|---|---|---|
| gcc -O0 比 | 1.13〜1.97x | **0.67〜1.02x** |
| gcc -O2 比 | 4.84〜7.41x | **1.08〜3.24x** |

**DESIGN N2(gcc -O2 比 2〜5 倍)は C カーネル 5 件すべてで成立した。** 3 つは
gcc -O0 より**速い** — -O0 が変数ごとにスロットを往復するのに対し、こちらは式の中間値を
レジスタに留めたまま次へ渡すからである。逆に言えば **-O0 同士の勝負はここで終わり**で、
これ以上は変数そのものをレジスタに置く(= 割付、第 2 段の
[register-allocation](../../issues/register-allocation.md))以外に伸びしろが無い。

saxpy 1 関数の命令数は **76 → 38**(gcc -O0 は 41)。

### 測ってから書いた結果、ベンチのコメントを 1 つ訂正した

`benchmark/c/arrayscan.c` は「gcc -O2 がベクトル化するので、この差はベクトル化と割付の
**両方**を測る」と書いていた。実測はその前半を否定した — gcc の `-O1 → -O2` は
全カーネルで **1.01〜1.16x** しかなく、`-O0 → -O1` が 1.04〜4.78x である。
**このコメントは推測で書かれ、測られていなかった**。訂正した。

**測定は単調時計で行うこと**。最初 `date`(壁時計)で測ったところ `treesum` が
**-0.063 秒**という負の値になった(このホストは `systemd-timesyncd` が時計を step させる)。

## register-allocation-1 — 1 つの値が 1 本のレジスタを関数の間ずっと持つ

### 測定が選択肢を 1 つ消した

着手前に `saxpy` を第 1 段(spill-traffic-cleanup)完了後の rubycc と gcc -O1 で比べた。
**内側ループが 22 命令 対 6 命令**で、残るスロット往復 11 か所は**すべてブロックを
またぐ値**だった — 誘導変数(毎周ロード 3 回 + ストア 1 回)と、ループ不変なのに毎周
読み直されるパラメータ 4 本である。**ブロック内で閉じる値は第 1 段で既にレジスタ常駐**
なので、issue が着手前の選択肢に挙げていた「まず基本ブロック内に閉じる」は、
**この実測で消えた**。

### 干渉を「解く」のではなく、起こさない

採ったのは**関数全域の専有割付**である。適格な vreg を選び、**関数の全区間**その
callee-saved レジスタに固定する。再割り当てを一切しないので、
**2 つの値が同じレジスタを欲しがることが構造的に起こらない** — 生存区間解析も
干渉グラフも spill 挿入も要らない。第 1 段と同じ取引(平坦な命令列から決められることだけ
決め、決められないものは断る)をもう一段行っただけである。

レジスタ利用効率は線形走査に劣る。だが空きは **x86-64 で 5 本(rbx / r12〜r15)、
AArch64 で 10 本(x19〜x28)**あり、どちらのバックエンドもそれまで 1 本も使っていなかった。

### 断る条件は 4 つ

`IR::Promotion` が対象外にするもの:

- **`:addr_of` の対象**。`&v` を配った以上、そのポインタ経由の読みはスロットに値がある
  ことを期待する
- **ベクタレジスタファイルに触れる値**。浮動小数点値も同じ 8 バイトスロットに住む規約
  なので、レジスタに載せると使うたびに 2 つのレジスタファイル間を移すことになる。
  **System V には callee-saved な xmm が 1 本も無い**ことも効く。第 1 版は整数と
  ポインタに絞った
- **可変長関数**。プロローグが引数レジスタをレジスタ退避域へ書く、命令列に現れない効果がある
- **未知の op を含む関数**まるごと(`IR::Simplify.known_op?`)。読み取り箇所の列挙に
  漏れがあると、誰も書かないスロットから読むコードを作ってしまう

**ベクタ条件は op の一覧だけでは足りなかった。** `:call` の引数 kind と戻り値 kind、
`param_kinds` の `:sse4`/`:sse8` は、**どの op にも現れないのに** movss/movsd で
スロットとの間を往復する。ここを落としていると **double 引数が壊れる**。

もう 1 つ、正しさではなく損得の除外がある — **transient(第 1 段が見つけた「次の命令だけが
読む値」)は候補から外す**。スロットに一度も触れない値を昇格しても、**存在しない 2 命令が
実在する 2 つの mov に置き換わる**だけで、レジスタも 1 本消える。

### 順序はループの重みで決める

候補は登場回数(読み + 書き)で並べる。**後ろ向き分岐(飛び先ラベルが自分より前にある
分岐)で囲まれた区間**を 10 倍に重み付けし、入れ子は掛け算になる。CFG は作らない。
`if` の両腕を二重に数えるし、分岐が本体の外にある書き方のループは見落とすが、
**答えは候補の順序でしかない**ので、近似の代償は「悪い選択」であって「誤った選択」ではない。
**同点は vreg 番号順**。選択が命令列だけの関数であることが N4 を守る。

### 退避は push でなく mov

callee-saved を使う以上、関数は借りたものを返さねばならない。**push/pop ではなく
フレーム内に 8 バイトの退避スロットを取って mov** する。こうすると rsp の 16 バイト整列が
**`sub rsp` 1 か所だけで決まり**、退避本数の偶奇に依存しなくなる。出口は `:ret` の 1 か所
(`leave` はバックエンド全体で 1 か所しかない)なので、復元漏れの経路が構造的に無い。
復元は**戻り値をロードした後・`leave` の前**に置く — 戻り値自身が昇格レジスタから来る
場合があり、かつ rbp がまだフレームを指している必要がある。

## register-allocation-2 — レジスタに置いただけでは速くならなかった

第 1 段を実装して測ったら、**saxpy の内側ループは 27 命令から 32 命令に増えた**。
メモリ参照は 13 か所から 2 か所へ落ちているのに、である。arrayscan は **+6.9% 遅く**なった。

原因は 2 つとも構造的だった。

- `#load_reg` / `#store_reg` を mov に差し替えただけなので、**`add eax,[rbp-96]` が
  `mov eax,r13; add eax,ecx` になる** — メモリ 1 命令がレジスタ 2 命令に置き換わり、
  命令は消えない
- 昇格 vreg を `#slot_written?` から外したため、**第 1 段で入れた「第 2 オペランドの
  スロット直接参照」がむしろ 1 つ消えた**

**値をレジスタに置くことと、レジスタを名指すことは別の作業だった。** 32 命令のうち
15 命令が `mov %rXX,%rYY` で、それを消すには昇格レジスタを**オペランドとして直接
名指す**必要がある。本ステップはそれである。

### r/m は「スロット」「昇格レジスタ」「どちらでもない」の 3 択になった

第 1 段の `#slot_written?`(スロットに書かれたか)を `#rm_operand?`(r/m として
名指せるか)に置き換えた。名指せないのは**transient だけ**である — スロットも
専用レジスタも持たず、生成側が使ったスクラッチにいるだけだからである。
`#emit_vreg_rm` が住処を見て `[rbp + disp]` か mod=11 かを選ぶ。

`cmp` は**両オペランドを読むだけで何も書かない**ので、reg フィールド側も昇格レジスタに
できる。`:sext`/`:zext` の source も r/m なので同じ話になる。

### 結果を昇格レジスタへ直接書く — 広げた所と見送った所

宛先が自由なフィールド(ModR/M の reg、`B8+rd` の下位 3 ビット)を持つ命令は、
結果を昇格レジスタへ直接書ける。`:const`・`:load`・`:sext`/`:zext`・`:scaled_add`、
そして `memory_bytes` を持つ二項演算がこれにあたる。二項演算で **`dst == a` なら
1 命令の in-place 更新**(`add %r12d,%ebx`)になり、`i = i + 1` と `sum += x` が
1 命令に畳まれる。可換演算は `dst == b` を先に交換して同じ形へ落とす。

**`dst == b` かつ `dst != a` の非可換形だけは除外**した。`a` を宛先レジスタへ入れる
move が、**まだ読んでいない `b` の住処を壊す**ためである。

**固定レジスタを要求する命令は広げなかった** — `idiv`/`div`(商 rax・剰余 rdx)、
シフト(cl)、`mul`(rdx:rax)、比較の結果(`setcc` が al を書く)、`:neg`(`F7 /3` に
自由な reg フィールドが無い)。ここで無理をすると、固定レジスタの退避という
**別種の正しさの負担**を持ち込むことになる。

### SIB の特殊符号化が 3 つ、初めて手の届く所に来た

`lea` はベース・インデックス・宛先の 3 つとも自由なので、昇格値がどれにも来る。
スクラッチ 2 本(eax/ecx)しか使っていなかった間は**一度も踏み得なかった**符号化が、
ここで初めて到達可能になった:

| 形 | 規則 |
|---|---|
| ベースが **r13 / rbp** | mod=00 の SIB ベース 101 は「**ベース無し + disp32**」の意味。`mod=01` + `disp8=0` にする |
| ベースが **r12** | SIB ベース 100 + REX.B で r12。mod=00 のままでよい |
| インデックスが **r12** | インデックス 100 は REX.X=0 なら「**インデックス無し**」。REX.X=1 で r12 になる |

3 つとも**バイト列テストで固定**した。差分テストでは決して見えない種類の欠陥である。

### 測り直した

saxpy の内側ループは **32 → 20 命令**(第 1 段前は 27、gcc -O1 は 6)。
インターリーブ計測(同一プロセスで 1 ラウンドごとに全変種を 1 回ずつ・9 ラウンド中央値・
単調時計)での変化:

| カーネル | 昇格前 | 昇格のみ | 本ステップ後 | gcc -O2 比 |
|---|---|---|---|---|
...
**受け入れ条件(gcc -O2 比 3.0x 以内)を 5 件すべてで満たした。**

**mandelbrot だけ +2.0% で、命令数は減っている**(内側 26 → 24、外側 6 → 2)。
このカーネルは double の依存鎖(store-to-load forwarding)が律速で、昇格が触れない。
ループ先頭のアドレスが動いているのでコードレイアウト由来と読んでいるが、
**そこまでは切り分けていない**。

### 残っている一手

**昇格レジスタをメモリオペランドのベースにする**(`mov (%r12),%r13d`)のは見送った。
saxpy が 20 → 16 命令になる見込みだが、ModR/M/SIB の特殊符号化を**第 2 の面**に
広げる変更であり、受け入れ条件を満たした今のうちに切り分けておく方がよい。

## register-allocation-3 — AArch64 では「置く」と「名指す」が 1 段で済んだ

第 2 段(x86-64)を 2 段に分けざるを得なかったのは**機械の都合**である。x86-64 は
2 オペランド機械で、値をレジスタに移す作業と、そのレジスタを命令に名指させる作業が
別々に要った。AArch64 のデータ処理命令は Rd / Rn / Rm が独立した 5 ビット欄の
3 オペランド形式で、ロード/ストアのベースも同じく自由な欄なので、**最初から
「昇格レジスタをオペランドと結果の宛先として直接名指す」形**で書いた。
`add x19, x19, x20` が 1 命令で書ける以上、x86-64 が通った「昇格しただけで
27 → 32 命令に増えた」中間状態は、この的には存在しない。

選択規則(`IR::Promotion`)はターゲット非依存なので**一行も変えていない**。
このバックエンドが足したのは、レジスタ集合・退避場所・符号化の 3 つだけである。

### 使うのは x19〜x28 の 10 本 — 外した 3 本には理由がある

AAPCS64 5.1.1 の callee-saved は x19〜x28。**x18 は使わない**(プラットフォーム
レジスタで、動いている OS の ABI が何に使ってもよいと予約されている)。
**x16 / x17(IP0/IP1)も使わない** — リンカは `bl` の飛距離が足りなければ veneer を
挿入してよく、veneer はこの 2 本を壊す。既存のスクラッチ(A=x9 / B=x10 / C=x11 /
ADDR=x12、アトミック用 x13〜x15)、引数・戻り値の x0〜x8、x29/x30 のいずれとも
交わらない。

### 退避スロットはフレームの一番上 — そこだけが「何も動かさずに置ける」場所

AArch64 のフレームは sp からの正オフセットで、下から **outgoing argument area →
保存レコード(x29/x30) → vreg スロット → stack objects** の順に積む。新しい領域を
足せる場所は**一番上しかない**:

- outgoing area は最下部でなければならない(AAPCS64 は callee が `bl` 時点の sp から
  スタック引数を読む)
- 保存レコードと vreg スロットは、本体のあらゆるオフセットが測られる基準である
- stack objects はその間に挟まる

一番上に置けば、**昇格の有無で他のオフセットが 1 バイトも動かない**。本数が奇数でも
問題にならない — 16 境界は最後の `align16(running)` 1 か所で決まる。フレーム末尾に
きっちり接していなければならない領域は可変長関数のレジスタ退避域だけだが、
**可変長関数には昇格が 1 本も無い**(`IR::Promotion` が丸ごと除外する)ので両者は出会わない。

退避は push ではなく `str`/`ldr`(x86-64 と同じ理由)。復元は `#emit_epilogue` に**書くのが
1 か所**だが、**出口は 1 つとは限らない** — `:ret` ごとにエピローグが生成されるので、
関数に `return` が複数あればその数だけ出る。全出口が復元を通ることは、
`:ret` 以外に関数を出る道が無いことから保証される。alloca 関数では `mov sp, x29` の**後**に置いた
— 退避スロットは x29 相対なのでどちらでも読めるが、**sp で読んでも正しい位置**に
揃えておく方が、2 つの場合が同じ形で読める。

### 直接名指しを広げた範囲

Rd/Rn/Rm が自由な命令はすべて: 算術・論理・`mul`/`umulh`・除算と剰余・可変シフト・
`neg`・`sxt*`/`ubfx`・`cmp` + `cset`・`:scaled_add`・定数の materialize・`:copy`・
`adrp`+`add` の記号アドレス・`alloca` の結果。加えて **x86-64 では見送った 2 つ**:

- **`ldr`/`str` のベースレジスタ**。ModR/M と SIB が「r13 をベースにすると disp が要る」
  「r12 をインデックスにすると REX.X が要る」といった特殊符号化を持つ x86-64 と違い、
  こちらは 5 ビット欄に番号を置くだけである
- **`cbz`** のテスト対象。ループの条件判定がロード無しの 1 命令になる

**x86-64 が固定レジスタのために諦めたものが、ここでは 1 つも要らない** — シフト量は
ただの Rm(cl ではない)、除算の商と剰余は任意の Rd(rax/rdx ではない)、比較の結果は
`cset` の自由な Rd(al ではない)。

**広げなかったのは ABI が固定する所**: 呼び出しの引数(x0〜x7 / v0〜v7)、戻り値
(x0/v0)、アトミックの exclusive 列、そして `:memcpy` — こちらはループが渡された
アドレス自身を進めるので、スクラッチでなければならない。

### 住処が 3 択になった分、残余(residency)の不変条件を 1 本立てた

`SlotResidency` は「記録してから 1 バイトも出していない間だけ信じる」表で、
昇格レジスタへ書く命令が表を生かしたまま通れるのは、**表のキーに昇格レジスタが
決して現れない**からである。x86-64 は「PROMOTION_REGISTERS はスクラッチ・引数・
戻り値のどれとも交わらない」という事実に寄りかかっていたが、こちらは `#emit_copy` と
`:mem` パラメータで**昇格レジスタへ直接ロードする**経路があるため、それだけでは足りない。
`#load_reg` が反対側から不変条件を守る — 宛先が昇格レジスタなら
`note_slot_loaded` ではなく `note_register_clobbered` を呼ぶ。

もう 1 つ、`#emit_instruction` の冒頭で `refresh_slot_residency` を 1 回呼ぶことにした。
**オペランドが全部昇格済みの命令は何もロードしない**ので、表を捨てる機会が無いまま
`#store_result`(表を「有効」に戻す)へ届いてしまう。冒頭で 1 回洗えば、どの命令でも
表は「真」か「空」のどちらかになる。

### 「有効に戻す」の前提を確かめていなかった — クロスレビューが見つけた欠陥

`#store_result` は昇格済みの宛先に対して `note_register_clobbered(reg)` を呼び、
表を有効に戻していた。この呼び出しが正しいのは「**直前に emit したバイト列が、
表の名指す対象のうち `reg` しか書いていない**」ときだけである。`#store_result` は
その前提を**確かめていなかった**。違反が 2 つあった:

| 場所 | 何が起きるか |
|---|---|
| `#emit_alloca` | `load_reg(A, size)` が「A は size を持つ」と記録した後、`add A,A,#15` / `and A,A,#-16` が **A を上書き**する。表が有効に戻ると、後続命令は A を元の size と誤認する — A には丸め済みサイズが入っている |
| `#emit_divmod` の剰余 | `sdiv C, ...` が **C を書く**。C に対する主張が残っていれば同じことになる |

**直し方は、既定を安全側に倒すこと**である。`#store_result` は既定で
`refresh_slot_residency` を通し(常に正しく、失うのは最適化だけ)、
**「`reg` しか書いていない」と言い切れる呼び出し側だけが `only_wrote:` で速い経路を選ぶ**。
両バックエンドの 25 か所を 1 つずつ判断して振り分けた。**将来ここに命令を足す人が
黙って壊せない向き**にしたのがこの変更の本体である。

x86-64 側(先にマージ済み)は 4 か所とも `target` しか書いておらず**健全だった**が、
落とし穴は同じ形で開いていたので同時に塞いだ。生成物が 1 バイトも変わらないことを
sha256 で確認している。

### 一時値をスクラッチから直接読む — 測って気づいた 6 命令

最初の実装は saxpy の内側ループを 31 → 21 命令にしたが、**21 のうち 6 命令が
`mov x10, x9`** だった。`#load_binary_operands` が一時値を A から B へ退避するのは
「もう片方のオペランドが A に入ってくるから」であって、**もう片方が昇格済みなら
誰も A に入って来ない**。`#operand_register` に「A か B が既に持っているならそれを使う」
を足して 6 命令が消えた(**15 命令**)。

答えるのを A と B に限ったのは C を除くためである — 剰余は途中で C に商を書くので、
そのとき C が読む予定の値を持っている可能性がある。

### 測った

`void saxpy(int *b, const int *a, int scale, int n) { for (int i = 0; i < n; i++) b[i] += scale * a[i]; }`
の内側ループ(2026-08-14、`aarch64-linux-gnu-objdump` で数えた):

| | 命令数 |
|---|---|
| 昇格前 | 31 |
| 本ステップ後 | **15** |
| `aarch64-linux-gnu-gcc -O1` | 7 |

**ループ内のスロット往復は 0 になった**。残る 15 のうち 2 命令は同じ `sxtw x9, w19` の
重複で、これは割付ではなく共通部分式の話である。

### テストは「スロットに置いたまま」を明示的に作る側になった

10 本もあると、テストが書く程度の小さな関数は**値が全部昇格してしまい**、
スロット経路の符号化テストが 1 つも走らなくなる。`test/test_aarch64_backend.rb` の
`#func` に `pin:` を足し、既定で全 vreg の `&v` を前置して**スロットに釘付け**にした
(実コードで昇格を断る条件そのものである)。釘の終わりは番兵の `:atomic_fence`
1 語で見つける — 本体側が出さない語であり、かつ**そこまでの residency を落とす**ので、
本体は釘が無かったときと同じ状態から始まる。昇格そのものを見るテストは `pin: []` を渡す。
...
### 測っていない列を「満たした」と書きかけた

`register-allocation` の受け入れ条件は「**`benchmark/run.rb` の全ケース**で gcc -O2 比
3.0x 以内」である。決着節に「1.09〜2.78x(C カーネル 5 件)」と書いて満たしたと判断したが、
**`benchmark/run.rb` の「全ケース」には json / msgpack が含まれる**。範囲を括弧書きした
ぶん嘘ではないものの、**条件が要求する分母を測らずに判定していた**。

測ると json は **4.98x** で、DESIGN N2(2〜5 倍)には収まるが**内部目標の 3.0x には届かない**:
...
### 別の課題の着手前計測が、自分の請求書を見つけた

`cross-process-header-cache` に着手し、まず現状値を再現しようとしたら、
`THROUGHPUT.md` の記録 13,854 行/秒に対し **11,063 行/秒**しか出なかった。

ペア計測(同一セッション 3 ラウンド交互)で **12.4% の回帰**と確定した。旧ツリー
(`94890f2`)は記録値をほぼ再現するので、ホストのドリフトではない。**原因は直前の
自分の作業** — `spill-traffic-cleanup` と `register-allocation` である。

**N1(コンパイル速度)と N2(生成コード品質)は正面から競合する。** 実行速度を
gcc -O2 比 7.65x → 4.98x にした代わりに、コンパイル速度を 12.4% 払っていた。

内訳は**ツリーを差し替えず**に取った — 同一ツリーで `IR::Simplify` / `IR::Promotion` を
段階的に無効化して交互計測すれば、差はパスそのものに帰属する。結果は
**IR パス 6.4〜12.2% / バックエンド 4.4〜7%** でほぼ半々だった。

### 数えるのは 1 回でよかった

1 関数あたり同じ命令列を **12〜15 回**走査していた:

| 処理 | 回数 | なぜそうなっていたか |
|---|---|---|
| `known_op?` の全走査 | 4 | `Simplify.run` / `Promotion.candidates` / `transient_vregs` × 2 が各自で確認 |
| `read_counts` | 5〜6 | 3 つの書き換えが**書き換えるたびに数え直していた** |
| `transient_vregs` | 2 | `Promotion` とバックエンドが**互いを知らずに**同じものを計算 |

`IR::Analysis` を新設し、**未知 op の有無・読み/書き回数・transient 集合を関数 1 本に
つき 1 回だけ**計算して `Simplify` → `Promotion` → バックエンドへ配る形にした。
3 つの書き換えは数え直す代わりに**差分更新**する。`operand_vregs` の
命令ごとの配列生成は yield 形に替えた(GC はボトルネック分析で 13.1% を占めていた)。

### 融合だけが事前スナップショットを要る

差分更新で 1 か所だけ非対称になった。**添字融合は、判定を「書き換え前の数え上げ」から
取らねばならない** — ループは**後方定義の値**を読むので、ある融合が読み回数を下げると、
それが後続の融合の `reads[mul.dst] == 1` という判定を動かしうる。前送りと死結果除去には
この性質が無い。コピーは**最初の融合が起きた瞬間だけ**払う(何も融合しない関数では
発生しない)。

### 死結果除去の打ち切り

不動点ループは「その周で読み回数が 0 に落ちた値が 1 つも無ければ、次の周で新たに
死ぬ命令は存在しえない」で打ち切れる。**減算しか起きない**からである。
1 周目は必ず全走査するので、最初から読み手のいない純粋命令を取りこぼすこともない。

### 受け入れは「1 バイトも変わらないこと」

これは純粋な内部リファクタなので、**生成物のバイト一致**を受け入れ条件にした。
x86-64 と AArch64 それぞれで benchmark・examples・c-testsuite の **532 ファイルを
sha256 比較して全一致**。「速くなったか」と「壊していないか」を別々に、機械的に
判定できる形にしたのがこのステップの要点である。

結果は **master 比 +11.1%**、最適化前比 0.864 → 0.960 で、**回帰の約 71% を回収**した。

### 残り約 4% は買ったものの値段である

stackprof で後半処理 41.7ms の内訳を見ると、**バックエンドの emit が 59%**。
最適化前ツリーは同じ部分が 16ms で、しかも**より長い命令列**を吐いていた。
差は命令ごとに実際に増えた仕事 — 残余表の更新、昇格 / transient の参照、
オペランド形の 3 分岐である。**これは最適化の本体であって無駄ではない。**
ブランチ上で両パスを無効化しても全体時間がほぼ変わらない(0.491s → 0.499s)ところまで
来ており、パスの費用がパスの減らした emit 量と相殺している。

### 空のログを「結果」と受け取りかけた

内訳を取る細工(`tmp/throughput_patched.rb`)を書いて 6 条件を回したところ、
**全条件が 0 バイトのログを残して「完了」した**。ハーネスが
`main if __FILE__ == $PROGRAM_NAME` で守られており、`load` では main が走らない。
マーカーだけが作られ、`grep` は何も拾わなかった。

**中身を見ずに次へ進んでいたら、誤った内訳で設計判断していた。** 計測を自動化する
ときは、「終わったか」ではなく「**測れたか**」を確かめる形にすること。

## verify-gem-test-deps-unpinned-1 — 台帳の合否が、実行順序に依存していた

コーパス再測(`corpus-reverify-1`)で、**同じコードに対して 2 回の実行が違う答え**を
出した。1 回目は date / etc / nkf / psych が PASS、15 分後の 2 回目は 4 件とも FAIL。
落ちていたのは gem の C 拡張ではなく**テストヘルパ**で、`test-unit-ruby-core 1.0.5` の
`assert_ractor` が `Ractor#value`(Ruby 3.5 で入ったメソッド)をこのホストの 3.4.5 で
呼んでいた。

**犯人は `racc` のレシピだった。** racc は自身の Gemfile に合わせて
`test_dep_versions` で 1.0.5 を固定している。ところが**スクラッチ GEM_HOME は全 gem で
共有**なので、racc が走った時点で**後続の gem も 1.0.5 に落ちる**。1 回目は 4 件が
racc より先に走って 1.0.15 で通り、racc が 1.0.5 を残し、2 回目は全 gem がそれを使った。

### 対照との比較では決着しなかった

このリポジトリの除外基準は「**対照 gcc も同じ数字で落ちる**」である。対照を回すと
4 件とも PASS した — **だがこれは決め手にならない**。`--control` は別の GEM_HOME を
使い、そこには 1.0.15 が残っていた。**「コンパイラの違い」を見ているつもりの比較に、
helper の版差が混ざっていた。** 決着させたのは、**コンパイラを変えずに版だけ戻す**
実験である(4 件とも PASS に戻った)。

**比較の道具そのものが、比較したい差以外を含んでいないかを確かめる必要がある。**

### 直し方 — 前の実行が何を残していても上書きする

**各 gem の実行直前に、その gem が使うべき版を入れ直す**。既定版は
`DEFAULT_TEST_DEP_VERSIONS` に 1 か所でまとめ、レシピの pin がそれを上書きする。
これで rubycc 側と対照側も同じ版になる。

検証は「**順序を変えても同じ判定になること**」で行った。1.0.5 と 1.0.15 が両方
キャッシュに残っている(最も再現しやすい)状態で 3 通りの順序を回し、5 gem すべてが
毎回 PASS した。

**版を決めた根拠と、ホストの Ruby が上がったときの直し方を定数のコメントに書いた** —
測り直して定数を更新する、**個別レシピを特例化しない**。特例化こそが今回の原因だった
からである。

## warning-directive-1 — `#warning` のために、報告して戻る診断を 1 本作った

`#warning`(C23 6.10.2p2、長く GNU 拡張)は rubycc では
`invalid preprocessing directive '#warning'` で**ビルドが止まっていた**。
gcc は警告 1 行を出して終了コード 0 で通る(実測 2026-08-19、gcc 13.3.0)。
移植性ヘッダが「未知のコンパイラです」と告げるためだけの行が、ビルドを殺していた。

### 難しかったのはディレクティブではなくチャネルだった

`#error` の隣に 1 ケース足すだけの作業に見えるが、そうではない。
このコンパイラの診断は **CompileError を raise する 1 種類しかなく**、
「報告する」と「この翻訳単位を諦める」が同じ動作だった。front/parser.rb に
`a warning is not a channel this compiler has` と 3 か所書いてあるのはその事実である。
`#warning` を受理するとは、**報告して戻る経路の 1 本目を作る**ことだった。

### 書式は 1 か所に置いた — severity だけが違う

`lib/rubycc/diagnostics.rb` に `Rubycc::Diagnostics` を置き、
`render(severity, description, filename:, line:, column:, source_line:)` を
**CompileError と共有**させた(`CompileError#build_message` はこれを
`"error"` で呼ぶだけになった)。警告は raise するものが無いので
`Diagnostics.warn` がストリームへ直接書く。**二重管理にしない**とは、
「同じ体裁の文字列を 2 か所で組み立てない」ということである。

### ストリームはプロセス全体の状態にした

警告が上がるのはプリプロセッサのディレクティブ走査の奥で、その呼び出し元
(Compiler、そして次にこのチャネルを欲しがるであろう parser と IR generator)は
ストリームを受け取らないし、受け取るべき理由も無い。**引数として通すと、
関係の無い層が全部ストリームを持つ**ことになる。

並行性は反論にならない。rmake の `-j` は各ステップを **fork した子で隔離する**
(`Rmake::Executor`)ので、2 つの翻訳単位がこの状態を共有することが無い。

未設定時は**その場で `$stderr` を引く**(束縛時ではなく発行時)。おかげで
`capture_io` を使うテストがそのまま効き、Driver は `Diagnostics.to(@err)` で
**注入されたストリーム**へ警告を寄せられる — rmake がプロセス内で Driver を
走らせる経路でも、警告が他の出力と同じ行き先に揃う。

### `-w` は honor し、`-Werror` は honor しない

判断を分けた理由は非対称である。

- **`-w` は honor する。** 出力しか変えられないので壊しようが無いうえ、
  そもそも**無視リストに入っていなかった**(小文字なので `-W*` の正規表現に
  当たらない)。`-w` を渡したビルドは `unknown option '-w' ignored` を浴びていた —
  `-w` が黙らせてくれと言っている当のノイズである。黙らせる範囲はコンパイラの
  診断だけで、ドライバ自身の `rubycc: warning: …` は残す。**gcc も同じ線を引く**
  (実測 2026-08-19: `gcc -w -c a.c b.o` は `#warning` を出さないが
  `gcc: warning: b.o: linker input file unused …` は出す)。
- **`-Werror` は honor しない**(受理して無視のまま)。honor すれば、
  **rubycc がたまたま実装している 1 個の警告だけでビルドを落とし**、gcc なら
  同じソースで出したはずの何百の警告については黙る、という**恣意的な部分集合の
  昇格**になる。しかも落とし方が最悪で、移植性ヘッダの
  `#warning "unrecognized compiler"` がまたビルドを殺す —
  **このステップが終わらせたかった故障そのもの**である。
  警告の集合と呼べるものが育ったら再考する。

理由はドライバの `#silently_ignored?` の上にそのまま書いた。**黙って無視しない**
ことがこの項目の受け入れ条件だったからである。

### メッセージは `#error` の形に揃えた

本体はディレクティブの残りトークンを 1 スペースで綴ったもので、
**マクロ展開しない**(gcc と同じ)。引用符は必須ではなく、剥がしもしない。
空なら自分の名前(`#warning`)を名乗る。`#error` と共有の
`directive_message` にしたので、**片方だけ書式が動くことが無い**。
`#error` の挙動は変えていない(先に `#warning` があっても止まる、をテストで固定した)。

### 検証

`test/test_preprocessor.rb` に 9 本(位置・引用符あり/なし・メッセージ無し・
非展開・**読み飛ばしグループ内は無報告**・`#error` は依然失敗・既定は標準エラーで
**標準出力を汚さない**・チャネルを閉じると破棄)、`test/test_driver.rb` に 6 本
(終了コード 0 とオブジェクト生成・`-E` の標準出力が汚れない・`-w`・`-Werror`・
`#error` は失敗)、`test/test_diagnostics.rb` に 3 本(**警告は severity 語だけが
...
## default-external-encoding-1 — C はソースをバイト列として読む

### 見つかり方

`mkmf-shell-free-conftest` の調査で、シェルの無いイメージを当たったときに出てきた
3 つの壁のうちの 1 つである。**`LANG` 未設定の環境で、同梱ヘッダの非 ASCII バイトを
読んだ時点で落ちる** — `CompileError` ですらなく Ruby のバックトレースが出る。

**最小環境固有ではない。ホストで再現する:**

```
LANG= LC_ALL= LANGUAGE= ruby -Ilib exe/rubycc -c /tmp/enc.c
-> scanner.rb:74: invalid byte sequence in US-ASCII (ArgumentError)
```

CI コンテナ・cron・systemd 配下など、**ロケールを設定しない環境はいくらでもある**。

### 読み出しを直すだけでは足りなかった

`File.read` を `File.binread` にする 4 か所の変更で終わる、と見積もっていた。
実測が 3 つ足りないと示した。いずれも**直したことで新しく壊れる**形である。

1. **`column_at` が `String#length` 依存だった** — バイト列にすると `length` が
   バイト数になり、診断のキャレット位置が全部ずれる。**「継続バイト(0x80-0xBF)で
   ないバイトを数える」**に置き換えた。正しい UTF-8 では文字数と厳密に一致し、
   UTF-8 でないバイト列でも定義され、**スパンをどこで切っても和が保たれる**
   (既存の増分カーソルが要求する性質)
2. **`:other` トークンがバイト単位になった** — `ss.getch` はバイト列に対して
   1 バイトを返すので、迷子の `あ` が 3 トークン = 3 エラーになり、キャレットが
   文字の途中を指す。N3 の「トークンの綴りは自分の報告した列から始まる」という
   不変条件も崩れる。先頭バイト + 継続バイトを 1 トークンにまとめた
3. **異なるエンコーディングの連結**が 2 か所にあった。診断の組み立て
   (`Diagnostics.render`)と、`-E` の出力の組み立て(`Driver#render_preprocessed`)で、
   ソース行(バイト列)とファイル名(ロケール由来)が**両方**非 ASCII だと
   `Encoding::CompatibilityError` になる。**どちらも変更が持ち込んだ経路**で、
   1 つ目は実装者が自分で、2 つ目はクロスレビューが見つけた。
   **バッファをバイト列にするだけでは塞がらない** — 非 ASCII を保持した BINARY
   バッファに UTF-8 の非 ASCII を足すと、向きが逆になるだけで同じ例外になる。
   連結する側の綴りも揃える必要があった

**列は文字数のまま**である。バイト列で読むことと、人に見せる位置を文字で数えることは
別で、後者を諦める理由は無い。

数え方は**文脈自由**にした — 「継続バイト(0x80-0xBF)でないバイトを数える」。直前の
バイトを見る規則にすれば不正入力でも正確になるが、**先頭バイトと継続バイトの間で
スパンを切ったときに前半と後半で数え方が食い違い、増分カーソルの累計がずれる**。
代償は「**単独の継続バイトが 0 として数えられ、2 つ並べば同じ列を報告する**」ことで、
これはテストに期待値として固定した。**不正なバイト列での列の正確さより、実装が
依存している不変条件を採った**という判断である。

### 副産物 — UTF-8 文字列リテラルは元から壊れていた

`"あいう"` を含むソースは、**ロケールに関係なく** master で落ちていた
(`lexeme_reader.rb`: `12354 out of char range`)。文字を `ord` してバイト列に
append していたためである。バイト単位で扱うようになり、**gcc と同じ生バイトが
`.rodata` に入る**ようになった。

これは issue の範囲外の修正だが、**サンプルで守る価値がある** —
`examples/m6/source_encoding_1_non_ascii.c` を足した。`test_examples.rb` が
gcc 差分で比較するので、**生バイトの一致が常時検証される**。

### 受け入れは「1 バイトも変わらないこと」

読み出し方の変更であって、コンパイル結果を変えてはならない。**57 ファイルすべてで
sha256 が一致**した。

**新しいテストが空回りしていないことも確かめた** — 無改変ツリーに
`test/test_source_encoding.rb` を置くと **15 中 12 が落ちる**。残る 3 件は欠陥の再現では
なく、`-E` の回帰ガードと、入力を直接バイト列で渡す単体テストである。テストを足した
ときは、それが**落ちるべき状態で落ちること**まで見ないと、通ったことに意味がない。

### 同じ欠陥が残っている場所は、その場で直さず切り出した

`.pc` / Makefile / Gemfile / リンカスクリプトの読み出しに同じ形が残る
(`locale-dependent-file-reads`)。**`.pc` の非 ASCII は実在し、Gemfile は UTF-8 の
コメント入りが普通**である。加えて、**引き金が逆向き**の別欠陥も見つかった
(`argv-encoding-classification`: ロケール**有り** × 引数のバイト列)。
どちらも `GAPS.md` に索引を置いた。

## env-less-shebang-1 — 起動の仕方を、環境に訊かずに決める

`exe/*` の shebang は `#!/usr/bin/env ruby` だった。**`/usr/bin/env` の無い環境では
コンパイラもツールも起動できない**(status 127)。DESIGN R5 が想定するミニマム環境が
まさにそれで、`mkmf-shell-free-conftest` の調査中に実イメージで踏んだ 3 つの壁の 1 つである。

### 環境を見て分岐する案を捨てた

案は 2 つあった。**A**: インストール済みなら RubyGems の binstub を使う。**B**:
`CC = "#{RbConfig.ruby} #{RUBYCC_EXE}"` と**常に自インタプリタ経由**で起動する。

**B を採った。** A は「ソースチェックアウトか、インストール済みか」で挙動を変える形で、
直前のステップで「**環境で結果が変わる状態を作らない**」と決めたばかりだった。
B には副産物もある — **`env ruby` が PATH から別の Ruby を拾う**潜在バグが同時に消える。

### 1 か所だけ B が通らない

**`PKG_CONFIG` は 2 語にできない。** mkmf は この値を `find_executable0` に通して
**1 個のファイル名として stat** し、さらに配列で spawn する。2 語にすると
「見つからない」と判定され、**pkg-config 対応が無言で無効化される**。

そこで、値はパスのままにし、**spawn の瞬間に** argv[0] が自 gem の `exe/` 配下なら
`RbConfig.ruby` を前置する形にした。置き場所は shim の `ShellFreeCommands` — 
**「どう起動するか」を既に所有している層**である。環境を見ないので、
分岐を作らないという方針は保たれる。

### 「誤識別しても遅くなるだけ」は成り立っていなかった

`$(CC)` の**先頭語**でツールを識別していたので、先頭語が `ruby` になると
rmake の in-process 置換(B3)が壊れる。最初の実装は正規表現でインタプリタを
推測し、外れたら外部 spawn に落ちるから安全だ、と考えた。**私もその説明を読んで
納得した。クロスレビューが誤りを示した:**

| 入力 | 実際に起きること |
|---|---|
| `CC = jruby /gem/exe/rubycc` | `jruby` **自体がツールとして登録**され、`exe/rubycc` が**引数として Driver に渡る** |
| `CC = ruby -Ilib /gem/exe/rubycc` | 先頭語 `ruby` がツールになり、**`-Ilib` が Driver に渡る** |
| `CC=gcc` の環境でレシピに `ruby /tmp/gcc …` | basename が一致し、**別物が in-process 置換される** |

外部 spawn への穏当なフォールバックではなく、**壊れた実行**である。

**推測をやめた。** `tool_programs` を **`tool_prefixes`** にし、`$(CC)` の展開値を
語に分けて**末尾のオプション語だけを落とした並び**をプレフィックスとする。
レシピの argv は**先頭がプレフィックス全体と一致したときだけ**置換し、
**一致した語数だけ drop** する。`LDSHARED = $(CC) -shared` の `-shared` は
プレフィックスから外れるので Driver に届く。

これで `jruby` でも `ruby -Ilib …` でも**定義上正しい**。一致しなければ外部 spawn で、
**そのときこそ「遅くなるだけ」**が成立する。**「保守的だから安全」は、
どう外れるかを数えて初めて言える。**

### 空白を含むパス

2 語にした以上、`RbConfig.ruby` や gem の導入先に空白があれば語分割で崩れる。
引用を入れ、**rubycc の splitter と RubyGems の `Shellwords.split` の双方で
同じに読める**ことを実測で確かめた。`GEM_HOME="/tmp/gem home 406"` で
`gem install json` が通ることまで見ている。

### 検証は「起きないこと」を測る形にした

- **囮の `ruby`** を PATH の先頭に置き、実行されたら記録して 99 で終了させる。
  修正後は 3 つの Ruby すべてで**起動回数 0**、HEAD では 1 回起動していた。
  「同じ Ruby が使われる」を間接的でなく**直接**測っている
- in-process 置換のテストは、CC の先頭語に**存在しないインタプリタパス**を置く。
  プロセス起動なら必ず失敗するので、`.so` ができることが in-process 実行の**証明**になる
- `pkg_config` のテストは `xsystem` の monkeypatch をやめ、**実 mkmf の `xpopen` 経路**を
  通す形にした。**このテストが効いていること**も、前置を一時的に無効化して落ちることで
  確かめてある

`dhi.io/ruby:4` では **127 が消え**、rubycc が自前の診断(jemalloc ヘッダ欠落 =
...
読むテキストはそれだけではない** — `.pc`、Makefile、Gemfile、リンカスクリプトが
`File.read` のまま残っており、ロケールの無い環境では最初の非 ASCII バイトで落ちる。
issue はその 4 か所を挙げていた。

直すこと自体は 4 行で終わる。**高くついたのは、直したことで逆向きに壊れた側だった。**

### issue に無い 5 件目が、いちばん実害が大きかった

`doctor/verified_gems.rb` が `JSON.parse(File.read(path))` で自前の台帳を読んでいた。
同梱している `data/verified_gems.json` の `notes` には約物の非 ASCII が入っているので、
**ロケールの無い環境では、ユーザの Gemfile を 1 行も見ないうちに `rubycc-doctor` が
必ず死んでいた**。issue が挙げた 4 か所は「そういう Gemfile を書いたら落ちる」だったが、
これは**プロジェクトの内容と無関係に落ちる**。導入判断で最初に叩かれるコマンドである。

ここだけはバイト列にせず `encoding: Encoding::UTF_8` と明示した。**JSON は UTF-8 だと
規格が決めている**(RFC 8259 §8.1)ので、推測する余地が無い。

### 「バイト列で読む」は、ロケール有り側に新しい故障を作る

実測すると、壊れた文字列の振る舞いはタグによって非対称だった。

| 文字列 | UTF-8 と連結 | 正規表現・`strip`・`split` |
|---|---|---|
| US-ASCII タグの非 ASCII バイト | **通る**(Ruby が互換とみなす) | 例外 |
| BINARY の非 ASCII バイト | **例外**(`Encoding::CompatibilityError`) | 通る |

つまり読み出しをバイト列に替えると、**ロケールが有る環境で今まで通っていた連結が落ちる**。
パス結合・変数展開・`Dir.children` の戻り・`-l` 名の 5 か所がそれで、
**修正の大半はこちら側**だった。

### 境界は `File.join` ではない。`==` と Hash キーも境界である

実装は `File.join` が落ちる箇所を塞いだ。**それでは足りなかった。**
...
**非 ASCII を含む文字列は、エンコーディングが違えば `==` も `eql?` も `hash` も一致しない。**
ASCII だけなら一致する。だからテストは通る — ASCII で書く限り。

これで実際に壊れていたのが rmake の明示 goal だった。

| ツリー | `rmake -f Makefile 日本` |
|---|---|
| HEAD | `goal` を出力 |
| 修正後(未修正時点) | **無出力・exit 0** |

goal は ARGV 由来でロケールのエンコーディング、ルール名は Makefile 由来で BINARY。
`resolve` が明示ルールを見つけられず `node[:recipe]` が nil になり、step が積まれないまま
正常終了する。**引数なし(既定 goal)なら Makefile 内部で完結するので動く** — 壊れるのは
コマンドラインから名前を渡したときだけで、それが `make` の普通の使い方である。

**元の欠陥は例外だった。修正が作った欠陥は沈黙だった。** ビルドドライバが「作ったつもりで
作っていない」状態を作るので、後者のほうが質が悪い。エンコーディングを揃える作業で
危険なのは、揃え忘れが**例外ではなく不一致として出る**ところにある。

同じ形が pkgconf にもあった。`.pc` のトークン(BINARY)を
`PKG_CONFIG_SYSTEM_INCLUDE_PATH`(ENV = ロケール)と `include?` で比較しており、
非 ASCII のディレクトリを指定すると**落とすべき `-I`/`-L` が落ちない**。
既定値は ASCII リテラルなので、環境変数で上書きしたときだけ現れる。

### 規則は 1 か所に置いた

同じ規則を 9 か所で言い換えると、9 通りに古びる。規則の本体は `lib/rubycc.rb` に 1 つ置き、
各サイトは「なぜここが境界か」を 1〜2 行だけ書く形にした。
`text.b unless text.encoding == Encoding::BINARY` が境界の綴りである
(既にバイト列なら全体を dup し直さない)。

### 直さなかったもの

`doctor/builder.rb` の `Open3.capture2e` の戻り値はロケール依存のままである。
用途が `.lines` / `.chomp` / `.reject` / `.join` だけで、**正規表現も `strip` も
transcode も通らないため例外にならない**ことを実測した。ファイルの読み出しではなく
子プロセスの出力なので、この課題の対象でもない。

**ただしこれは用途に依存した安全性である。** ここに正規表現・`strip`・transcode の
いずれかが足されれば、同じ境界で再び落ちる。

### 検証

- `bundle exec rake test` — **3363 runs / 13324 assertions / 0 failures / 0 errors / 41 skips**
  (`origin/master` にリベースした後の実測。この変更だけを載せた時点では 3357 runs)
- 生成物は `benchmark/c/*.c`(5)と `examples/m6/*.c`(7)の **12 本すべて sha256 一致**
  (HEAD の複製と最終ツリーの双方でコンパイルして比較)
- 新テストは 17 件。**無改変ツリーでは 11 件が落ちる**(報告されていた欠陥)。
...
## argv-encoding-sibling-clis-1 — 残り 2 つのコマンドを同じ規則に揃える

`argv-encoding-classification-1` は `rubycc` 本体だけを直した。**自前で ARGV を分類する
コマンドは他に 2 つある** — `rmake` と `rubycc-ar` で、どちらも同じ形で落ちていた。
`rubycc-pkgconf` は生の引数に正規表現を当てないので落ちず、`rubycc-doctor` が出す
バックトレースは Ruby の `optparse.rb` のものなので、どちらも範囲外にした。

境界は本体と同じ考え方で置いた。rmake は `CLI#run(argv)` の先頭 — `parse_argv` ではなく
`run` にしたのは、`options[:file]` が直後の `File.expand_path` に出ていくので、
**分類と解決を 1 か所で覆える**からである。`rubycc-ar` は `ARGV` を `args` に写す行。
`--version` の判定はそれより前に置いたままにした(ASCII 比較しかせず、本体の `--version`
と同じく**順序が安全性の理由**になっている)。

### 起票時の実測は 2 か所足りなかった

issue の表は「`-f <bad>` は診断で済む」としていたが、**それは分離綴りだけ**だった。
joined の `-fM<不正バイト>` は捕捉の正規表現に当たって落ちる。さらに **bare `-j` の
次の語**も `cli.rb:98` の `nxt =~ /\A\d+\z/` で落ちる。**同じ「分類が正規表現」という
1 つの原因が、1 つのメソッドの中に 3 か所あった**ということである。

### 壊れる側は 2 つ、どちらも先行ステップと同じ形

**1. `File.expand_path` に相対パスを渡す経路。**`-f` の値をバイト列にすると、基準の
`@dir`(`Dir.pwd` 由来)と綴りが食い違い、作業ディレクトリ名が非 ASCII だと落ちる。
`dir_bytes` を足して基準側も揃えた。`Makefile` が既に同じ考え方の `dir_bytes` を
持っていた(`makefile.rb:353`)ので、命名をそれに合わせている。**本体では
`absolute_path` として現れたものが、ここでは同じ形で 1 段浅く出た。**

**2. アーカイブのメンバ名の突き合わせ。**`exe/rubycc-ar` の `ar r` は、アーカイブ内の
既存メンバ名を鍵にしたハッシュへコマンドライン側の basename を引き当てて、置換か追加かを
決める。ARGV だけをバイト列にすると、**非 ASCII の名前で一致しなくなり、置換すべき
ところが重複追加になる**。例外ではなく沈黙である。

### `ArReader` の非対称は、直さずに CLI で吸収した

原因はリーダ側にあった。**同じ 1 本のアーカイブでも、名前の長さで綴りが変わる**
(実測、2026-08-25):
...
短い名前はヘッダに直接入るのでバイト列のまま返り、長い名前は名前表から取り出すときに
`force_encoding(Encoding::UTF_8)` を通る(`ar_archive.rb:204`)。

**この非対称は master の時点で既に実害を出していた。**ARGV が UTF-8 だった master では
**短い**非 ASCII 名の `ar r` が重複追加になっており、長い名前だけが偶然一致していた。
つまり貼り直しは**短い名前を直し、長い名前を壊す**関係にあり、**両側を揃えて初めて
両方通る**。

リーダの戻り値はリンカの遅延展開も消費するので、**リーダは変えず CLI 側に `member_key` を
置いた**。`ar x`(展開)の突き合わせも同じ形だったので同時に揃えている(master では
`x <非 ASCII 名>` が無出力・exit 0 で沈黙していた)。リーダ側は
[ar-reader-name-encoding](../../issues/ar-reader-name-encoding.md) へ切り出した。

### 検証

- `bundle exec rake test` — **3395 runs / 13628 assertions / 0 failures / 0 errors / 41 skips**
- 生成物は `benchmark/c/*.c`(5)と `examples/m6/*.c`(7)の **12 本すべて sha256 一致**
- 新テストは 13 件。**無改変ツリーでは 9 件が落ちる**(8 failures + 1 error)。
  内訳は**例外が 6 件**(`cli.rb:93` の 4 経路、`cli.rb:98`、`exe/rubycc-ar:46`)と
  **沈黙が 3 件**(`ar r` の重複追加、`ar x` の無出力・exit 0、相対 `-f`)。
  通る 4 件は**性格が違うので分けて書く**(最初はまとめて「戻せば落ちるガード」と
  書いていたが、クロスレビューの指摘で 2 件がそうでないと分かった):
  **この変更の書き方を縛るもの**が 1 件 — 呼び出し元の配列を書き換えないこと
  (`map` を `map!` に戻すと落ちる)。**前ステップが立てた不変条件を、この変更が
  壊していないことを見るもの**が 1 件 — 非 ASCII の goal が Makefile の綴りと
  一致し続けること。`Makefile#plan` の `goal.b` は `locale-dependent-file-reads-1` で
  入っているので**無改変ツリーでも通り、この変更の差分証拠にはならない**。
  残る 2 件(分離綴りの `-f` の診断、アーカイブ名の往復)は**純粋な回帰ガード**である

### クロスレビューが直させたもの

別系統(codex)の事前クロスレビューは**ブロッカーを挙げなかった**が、**この記録自体の
事実誤認を 1 件見つけた** — 上の「通る 4 件」を一括して「対応する修正を戻せば落ちる」と
書いていた点である。実際には 2 件がそうではなく、**差分の証拠と将来の回帰ガードを
混ぜて書いていた**。記録を分けて書き直した。**測り方ではなく、測った結果の言い換え方が
間違っていた**という形なので、指摘されなければ残っていた。

テストの穴も 3 件指摘され、1 件を採った — `ar t` が**素の `m.name` をそのまま印字する**
唯一の経路なのに、列挙されるメンバ名が ASCII しか無かった。リーダの非対称が出るように、
短い名前と長い名前の両方を非 ASCII で列挙する形に広げた。

採らなかったのは 2 件。**`VAR=value` の値がターゲット名へ展開される経路**は、実装は
`Parser` と `Makefile#plan` の両方で揃っているが、テストは recipe への展開しか見ていない
という指摘である。妥当だが、この issue の受け入れ条件は goal と override の**分類**で
あって変数展開の意味論ではない。**executor の in-process 経路を非 ASCII の作業ディレクトリで
踏む**テストも同じ理由で見送った(`Driver` 側は `argv-encoding-classification-1` の
相対パステストが押さえている)。

## system-include-path-encoding-1 — 探索路の綴りを、合流する 1 か所で揃える

同じ形の 3 つ目である。`default-external-encoding-1` が読み出しを、
`argv-encoding-classification-1` と `argv-encoding-sibling-clis-1` が引数をバイト列にした。
残っていたのは**rubycc 自身が持っているパス**で、ヘッダの探索路がそれである。

`@include_paths` は**2 つの出所が合流する唯一の場所**だった。呼び出し元の `-I` は
driver 経由ならバイト列だが、同梱ヘッダと libc のディレクトリは `__dir__` から作るので
**プロセスの綴り**を持つ。ヘッダ名はソース由来でバイト列なので、
`File.join(dir, name)` が両方非 ASCII だと落ちる。

引き金は 2 つ揃ったときだけである — **rubycc 自身が非 ASCII のパスに置かれている**ことと、
**非 ASCII のヘッダ名が `-I` のどこにも無い**こと。後者は**綴りを間違えただけでも起きる**。
探索は `-I` を尽くしてからシステムディレクトリへ進むので、「見つからない」経路は必ずそこを通る。

### 直す位置を 3 案から選んだ

| 案 | 採否 |
|---|---|
| `File.join` の呼び出し側(`search_include_paths` と `include_exists?`)で揃える | **不採用**。同じ規則を 2 か所で言い換えることになり、`File.join` が増えるたびに増える |
| `BUNDLED_INCLUDE_DIR` などの定数の定義時に揃える | **不採用**。定数と `default_system_include_paths` の戻り値を突き合わせている既存テストに波及し、かつ**呼び出し元が UTF-8 で渡した `-I` を覆えない** |
| **`@include_paths` を組み立てるときに揃える** | **採用**。2 つの出所が合流する唯一の場所で、下流の `File.join` が何か所あっても効く |

`@system_include_paths`(同一性の判定に使う方)は `argv-encoding-classification-1` で
既に `absolute_path` を通っており、触っていない。

### 「引き金はテストでは作れない」は思い込みだった

最初は近似で済ませた。`include_paths:` に UTF-8 タグの非 ASCII ディレクトリを渡せば
同じ形の `File.join` になるので、**ツリーを動かさずに済む**という理屈である。
引き金そのもの — rubycc 自身が非 ASCII のパスに置かれていること — は
「テストの中では作れない」と判断していた。

**クロスレビューがそこを突き、測ったら間違っていた。**移すのに要るのは
`lib/` `include/` `exe/` の 3 つだけで、**2.3 MB・148 ファイル・27 ms** である。
「重いから近似で」という前提が成り立っていなかった。

そこで**引き金そのものを組み立てるテスト**を足した。3 つのディレクトリを非 ASCII の
一時ディレクトリへ複製し、子プロセスの rubycc を走らせる。**コピーであってリンクではない** —
`__dir__` はシンボリックリンクを解決するので、リンクでは元の ASCII のパスが返って
何も試さないことになる。覆うのは issue の受け入れ条件そのままの 2 件で、
**同梱ディレクトリにしか無い非 ASCII のヘッダが解決すること**と、
**どこにも無いヘッダが診断になること**(バックトレースではなく)である。

近似の側も残した。1 ディレクトリ単位の挙動(`__has_include` を含む)はそちらの方が
細かく見られる。加えて**「探索路の全要素がバイト列である」ことを直接 assert した** —
近似では覆えないシステムパス側の綴りが外れれば、これが落ちる。

### クロスレビューの指摘と、採らなかったもの

別系統(codex)の事前クロスレビューは **must-fix を 1 件**挙げた — 上記のとおり、
**issue の受け入れ条件が「非 ASCII のパスに置いた rubycc」なのに、テストがそれを
end-to-end で確かめていない**という指摘である。**この指摘は正しく、こちらの判断が
間違っていた。**修正の位置(3 案の比較)については、独立に同じ結論に達している。

**採らなかったのは 1 件** — 非 ASCII の同梱システムパスからの `#include_next` と
`system_header_redefinition?` を検査するテストが無い、という指摘である。どちらも
`absolute_path` を通る同一性の鍵を見ており、**そこは `argv-encoding-classification-1` で
既に揃えてあってこのステップでは触っていない**。`#include_next` の非 ASCII は
同ステップが `-I` 経由で押さえている。**このステップが変えた探索路の綴りとは別系統**なので、
ここで足すと「変更点を覆うテスト」ではなくなる。

テストの書き方(`define_method` での生成)が兄弟 2 ファイルと揃っていない、という指摘も
受けて、素の `def test_` + 内側のループに直した。3 ファイルで 1 つの族なので、
読み手が同じ形を期待できる方を採った。

### 検証

- `bundle exec rake test` — **3401 runs / 13660 assertions / 0 failures / 0 errors / 41 skips**
- 生成物は `benchmark/c/*.c`(5)と `examples/m6/*.c`(7)の **12 本すべて sha256 一致**
...
### クロスレビューを待たず、逆アセンブルで確かめた

別系統(codex)への事前クロスレビューは**遅れて返った**。1 度目は 15 時間返らず
(プロセスを見ると別の worktree のものだった)、2 度目も「待機中」の通知が 2 回続いた後、
3 回目でようやく本文が届いた。**指摘は 5 観点すべてで無し**である。

待つ間に、**読解ではなく逆アセンブルで**危ない所を潰した。結論は一致したが、
確かめ方が違う — codex はフィールドを読んで検算し、こちらは出た機械語を復号させた。

- 生成した `.o` を `objdump -d` にかけ、**両ターゲット・両幅の 4 系列**が意図した
  SWAR になっていることを確認した。x86-64 の 32 ビットは
  `shr/and 0x55555555 → sub → and 0x33333333 ×2 → add → shr 4/add → and 0x0f0f0f0f
  → imul 0x01010101 → shr 24`、64 ビットは同じ形で `shr 56`。
  AArch64 も同型で、`mul` は Ra=xzr の MADD として逆アセンブルされた
- **逆アセンブルできること自体が符号化の検査**である。手で組んだ語が不正なら
  objdump が別の命令か不正命令として読む
- 最終段の桁溢れは幅によらず安全である。1 バイトあたりの計数は最大 8 で、
  乗算後の最上位バイトに集まる和は 32(4 バイト)/ 64(8 バイト)、どちらも 8 ビットに収まる
- `emit_mul` を寄せた `MADD_ZERO` は `{ 32 => 0x1B007C00, 64 => 0x9B007C00 }` で、
  **元の直書きと同じ値**である。生成物の sha256 一致と合わせて二重に確かめた
- 定数畳み込みは幅でマスクしてから数える。`__builtin_popcount(-1)` が gcc と同じ 32 に
  なることを、配列長 3 本の実行差分(rubycc / gcc とも終了コード 72)で確認した

**codex 側の注意書きも残す。** レビュー中にこちらが rebase とコミットを進めたため、
codex は「読んでいる最中にツリーが変わった」と報告している(ステージ済みの状態から
コミット済みの状態へ)。読んだ内容は同じ変更だが、**レビュー中はツリーを止める**方がよい。
...
## roaring-out-of-scope-1 — 3 つの gap を炙り出した候補を、追加せずに閉じる

**内容**: `roaring` 0.4.1 を正式追加しないと決め、`OUT-OF-SCOPE-GEMS.md` へ
**除外基準 G(無効化できない経路でベクトル組み込み関数を使う)**として記録した。
**再検討の条件も同じ節に書いた。** ユーザ判断(2026-09-10)。コードの変更は無い。

**判断の対象は gem ではなく形である。** roaring だけを名指しで外すと、次に同じ形の gem が
来たときにまた一から調べることになる。外したのは
「**gcc と同じ枝を取った上で**ベクトル経路が必須になる」形で、
**ゲートで落とせる SIMD は対象内のまま**である — コーパスの多くの gem は
`arm_neon.h` / `cpuid.h` を probe の裏に置いており、probe が失敗すればスカラ経路になる。
roaring が違うのは `roaring.h:157` の `#if defined(__x86_64__) || defined(_M_X64)` で
`CROARING_IS_X64` が立ち、gcc も同じ枝を取って `__attribute__((target("avx2")))` と
実行時ディスパッチで本当に AVX2 を積む点である。

**選択肢は 3 つあり、保留が最も高くついた。**

| 案 | 評価 |
|---|---|
| SIMD 組み込み関数を実装する | **M2〜M4 級の規模**。要求もコーパスからの圧力も、この 1 件では足りない |
| **対象外として記録する** | **採用**。次に候補を見る人が同じ調査を繰り返さない |
| 保留を続ける | 判断が出ないまま調査だけが繰り返される。**現状がこれだった**(2026-08-26 から) |

**再検討の条件**(issue が「書け」と要求していたもの): 同じ形の gem がもう 1 件以上現れたとき、
rubycc が別の理由でベクトル型を持つことになったとき、roaring 側が `ROARING_DISABLE_X64` を
`extconf.rb` で選べるようになったとき。**いま落とせないのは gem のビルド設定の問題**であって、
rubycc の側の問題ではない。

**追加できなかったことと、候補として無価値だったことは別である。** この候補は
**実在の gap を 3 つ炙り出して役目を果たした** — `#warning`(PR #84)、
`__BYTE_ORDER__` ほか定義済みマクロ 5 件(PR #105)、同梱 `cdefs.h` の `__attr_*`(PR #106)。
いずれも roaring とは独立に価値があり、最小 fixture つきで別 PR に分離済みである。
...
## release-v1-1-0-1 — 配るものを作って動かしてから「済」にする

**内容**: v1.1.0 のリリース準備。`lib/rubycc/version.rb` を 1.1.0 へ、`CHANGELOG.md` に
1.1.0 エントリ、`RELEASE-CHECKLIST.md` に §6(v1.1.0)を追加した。
**タグ push と `gem push` は行っていない** — 自動化しない方針(アカウント保有者の操作)。

**版の選び方**: v1.0.0 から 84 本の PR が入ったが、**破壊的変更は無い**。
このプロジェクト独自の規則である「**コーパス合格率の低下は破壊的変更**」も、
率は 91.2% → 94.3% と上がっているので該当しない。よって **minor**。

**設計判断**:

- **`CHANGELOG.md` は英語で書く。** この文書は gemspec の `files` に入っていて
  **gem に同梱されて配られる**ので、読み手は日本語話者に限らない。1.0.0 のエントリも英語である。
  最初 日本語で書いてしまい、1.0.0 を読み返して気付いて直した。
  **「ドキュメントは日本語」という規約は、リポジトリの中を読む開発者向けの文書の話**であって、
  配布物に同梱されるものは読み手が違う。
- **チェックリストの警告に従って、順序を変えなかった。** §5 はこう書いている —
  「この表を『済』で埋めても**配るものが正しいとは限らなかった**。実際にタグを打つ前に
  `gem build` と distroless を走らせたところ 3 件の不具合が出ている」。
  今回も**ビルドした gem を隔離した `GEM_HOME` に入れ、コマンドを起動し、
  実際に C をコンパイルして実行してから**表を埋めた。

**測ったこと**(2026-09-12):

| | 実測 |
|---|---|
| `rake test` | **3,498 runs / 0 failures / 0 errors / 39 skips**(`ci_check_skips` も OK) |
| 再現ビルド | `SOURCE_DATE_EPOCH=1789176602` で 2 回ビルドし **バイト一致**(637,440 bytes) |
| 同梱物 | 165 ファイル。必須 5 点すべて在り、ヘッダ 81 / lib 66 / exe 5 |
| 配るものの実動 | `rubycc` / `rubycc-ar` / `rubycc-doctor` が `1.1.0` を答え、`hello.c` が**コンパイルして実行できた** |

**前回の不具合は再発していない。** v1.0.0 の準備で見つかった `rubycc-doctor --version` の
"version unknown" と `CHANGELOG.md` の同梱漏れは、どちらも今回の確認で正しい状態だった。
`rmake` と `rubycc-pkgconf` が `--version` に答えないのは **§5 で「直さない」と決めたとおり**で、
mkmf がその引数を渡さないこと、pkg-config の `--version` は本物ならツール自身の版を返すもので
**似て非なる値の方が危険**であることが理由である。

---

## release-v1-1-0-record-1 — 配った現物と、検証した現物を一致させる

**内容**: v1.1.0 の公開を記録した。タグ `v1.1.0`(`6c9ded2`)と `gem push` はどちらも
**リポジトリ所有者が手で実施**し、[run 34672677229](https://github.com/nuna/rubycc/actions/runs/34672677229)
(Tier C 全ジョブ success)と rubygems.org での公開を確認してから
`RELEASE-CHECKLIST.md` §6 の 2 行を済にした。あわせて ROADMAP の「現在地」にある
検証済み gem の件数を 31 → **33** に更新した(この節は**現時点の事実だけ**を置く約束である)。

**配った gem は、準備中に手元で検証した gem とバイトが違う。** `SOURCE_DATE_EPOCH` は
コミット時刻に固定されるので、準備中の検証(マージ前、`1789176602`、637,440 bytes)と
Tier C のビルド(マージコミット、`1789177496`)では**入力が違う**。
再現性の主張は「**同じツリーを同じ epoch で 2 回ビルドすると一致する**」であって、
両者はそれぞれ独立にそれを満たしている。**齟齬ではないが、取り違えやすい**ので
チェックリストに節を立てて書いた。

**配るのは Tier C の artifact である。** 手元で作り直せばコミットが同じでも
ビルド環境の差が入りうる。**CI が「2 回ビルドしてバイト一致」を確認した現物を
そのまま push する**のが、検証したものと配ったものを一致させる唯一の形である。

**公開の確認**(2026-09-12):

| | 値 |
|---|---|
| rubygems.org | `rubycc 1.1.0` / platform `ruby` / 2026-09-12T04:30:58Z / SHA-256 `ea2adb59…` |
| Tier C | `Rubycc::VERSION=1.1.0  tag=v1.1.0 (-> 1.1.0)`、再現ビルド検査つきで success |

---

## scan-rank-thousands-separator-1 — 拡張の前に、探す道具が 1000 で止まっていた

**内容**: `tools/scan_popular_gems.rb` の bestgems 行パーサが**桁区切りのランクを読めない**
バグを直した。`ROW_RE` のランク捕捉が `(\d+)` で、`1,000` の行に一度もマッチしない。
**コーパスを 100 件へ広げる指示を受けて最初に走らせた走査(ランク 501〜1000)が、
最後のページで落ちた**のがきっかけである。

**症状が原因を指していなかった。** 落ち方は

```
https://bestgems.org/total?page=50: page says ranks 981-1000 but 19 rows were extracted
```

で、メッセージは**ページの体裁が変わった**ことを示唆する。実際にはヘッダ側の正規表現は
`([\d,]+)` で桁区切りを許しており、**行側だけが許していなかった**。
つまり自分のパーサの中で 2 つの数値の読み方が食い違っていた。

**この穴はちょうど作業の前方にあった。** ランク 999 までは症状が出ない —
**4 桁になる 1000 位から先が、これから掘る範囲そのもの**である。
上位はすでにコーパスに入っている gem が多く、新しい候補は下位にしかない。

**設計判断**: ランクも `([\d,]+)` で受けて `Integer(rank.delete(","))` に統一した。
**ヘッダと行で同じ読み方をする**ことがこの修正の要点で、片方だけ直すと同じ形の穴が残る。

**検証**: ランク 999 と 1,000 の 2 行を含むフィクスチャで回帰テストを追加した
(修正前は 1 行しか取れず、行数検査で落ちる)。
`test/test_scan_popular_gems.rb` は **29 runs / 0 failures**。

---

## corpus-expansion-plan-1 — 「増やす」と「コーパスに入れる」を同じ操作にしない

**内容**: 「**ビルドできる gem を 100 件まで増やす**」というユーザ指示(2026-09-12)に対し、
**そのまま実行すると何が壊れるか**を数え、進め方を決めて 2 件起票した。コードの変更は無い。

**数えたら、指示をそのまま実行できないことが分かった。** 合格率の分母は
`test/corpus/gems.rb` にあってゲートを通った gem で自動的に決まり、分子は
`data/verified_gems.json` に (d) 水準の記録があるものである:

| | 分母 | 分子 | 率 |
|---|---|---|---|
| 現状 | 35 | 33 | 94.3% |
| **100 件足して検証しない場合** | 94 | 33 | **35%** |
| 100 件で R10 を満たす場合 | 100 | **90** | 90% |

しかも「**合格率の低下は破壊的変更**」がこのプロジェクトの独自規則である。
**「ビルドできる」と「コーパスに入れる」を同じ操作にすると、R10 の意味ごと壊れる。**

**費用も 1 桁違う**(実測):

| 水準 | 1 件あたり |
|---|---|
| build_load(install して `.so` がロードできる) | **1〜2 分**(ほぼ自動) |
| (d)(上流テストスイート合格) | **20〜40 分**(`bindex` はピン特定で 3 回やり直した) |

**ユーザ判断は A + C** — コーパス(R10 の分母)は (d) 水準のまま、
「ビルドできる」は**別の台帳**に持つ。[buildable-gems-ledger](../../issues/buildable-gems-ledger.md) に起票した。

**探索が律速だったので測った。** 候補はランク下位にしか残っていないので数千ランクを
掘る必要があるが、走査は gem を**丸ごと落としてから**判定していた。
`.gem` は tar で **`metadata.gz` が先頭メンバ**、そこに `extensions:` と `files:` がある。
rubygems.org は Range 要求に応える(HTTP 206 を実測)。人気 gem 10 件で比べると:

| | バイト数 |
|---|---|
| 全文(いまの走査) | 5,818,880 |
| 先頭 8 KB だけ | **81,920(1.4%)** |

[scan-metadata-prefilter](../../issues/scan-metadata-prefilter.md) に起票した。

**供給も測った**(走査はランク 501〜1500 の 1,000 件、2026-09-12):

| | 件数 |
|---|---|
| C 拡張を持つ | 54 |
| **ゲート通過・未収載 `[1]`** | **43** |
| ゲート除外 `[2]`(C++ / configure) | 8 |
| 要レビュー `[R]` | 5 |
| 既にコーパス内 `[3]` | 3 |
| C 拡張なし | 940 |

**上位ほど痩せている。** ランク 101〜500 では新規候補が 9 件しか出なかったのに、
501〜1500 では 43 件出た — **上位の C 拡張 gem は既にコーパスに入っている**ためである。
既知の候補は合計 52 件で、いまのコーパス 41 件と足して 93 件。**100 件に届かせるには
ランク 2000〜2500 まで掘る必要がある**うえ、52 件の全部が通るとは限らない
(`rbtrace` は `sed`/`awk` 依存で落ちることが分かっている。`libv8` と `sass-embedded` は
バイナリを取りに行く形、`commonmarker` と `prometheus-client-mmap` は C ファイルが
0 件で Rust 拡張と見られる)。

**1,000 ランクの走査に 1 時間・1.8 GB かかった。** 前処理を入れる理由はここにある。
**深い検査は全文側に残す** — `bcrypt` の除外根拠(`extconf.rb` の `$objs` に `x86.o`)は
`data.tar.gz` の中にあり、metadata だけでは判定できない。第 1 段は
「落としてよいか」ではなく「**全文を取る価値があるか**」の判定に留める。

---

## scan-metadata-prefilter-1 — 落とす前に、落とす価値があるかを訊く

**内容**: 候補走査を 2 段にした。第 1 段は `Range: bytes=0-8191` で `.gem` の先頭だけ取り、
**先頭メンバである `metadata.gz`** から gemspec を読んで「全文を取る価値があるか」を決める。
第 2 段(全文取得・SHA-256・展開・深い検査)は**第 1 段を通ったものだけ**に走る。
実装は heavy-implementer。コーパスを 100 件へ広げる作業の律速だった。

**効果**(ランク 101〜300、空の `SCAN_WORK` から。2026-09-12 実測):

| | 前 | 後 |
|---|---|---|
| 実時間 | **10m42s** | **2m39s**(4.0 倍) |
| 全文取得 | 200 件 | **20 件** |
| 省いた転送 | — | **35,839,291 バイト** |
| work dir | 305,972,203 バイト | **83,564,010 バイト** |

**設計判断**:

- **`extensions` が空でも `files` を見る。** 「拡張を宣言していないなら C は無い」で切ると、
  **`[R]`(要レビュー)の経路が消える** — いまの走査は「gemspec は宣言していないが archive に
  C ファイルがある」gem をそこで拾っており、実測で `libdatadog` / `pry-doc` /
  `datadog-ruby_core_source` の 3 件が該当した。`gem build` は gemspec が並べたファイルだけを
  詰めるので、`files` は展開後に見る集合と同じものを指す。
- **拡張子の一覧は意図的に上位集合**にした。余分に入れても**全文取得が 1 件増えるだけ**で、
  足りないと**候補が黙って消える**。誤りの代償が非対称なので、寄せる向きは決まっている。
- **断片で切れたメンバは例外にせず「あと何バイト要るか」を返す。** その数値があれば
  ぴったりの長さで 1 回だけ取り直せる(実測: 200 件中 4 件が 8 KB に収まらなかった)。
- **`required_ruby_version` が走行中の Ruby を満たさないなら、プローブを捨てる。**
  第 1 段は v1 API の「最新」を読み、第 2 段は `gem fetch` が解決するリリースを取る。
  最新が Ruby の要求を上げていると**両者は別のリリースを見る**ので、読んでいない中身について
  プローブの答えを流用してはいけない。**古いリリースこそ、取り除かれる前の C 拡張がある場所である。**
- **platform 判定とは寄せなかった。** platform は v1 API のリリース情報だけで分かるので
  **prefix を送る前**に弾ける。`required_ruby_version` は gemspec の中にしかなく、
  prefix を取った後でしか分からない(v1 の `/api/v1/gems/<name>.json` に
  `required_ruby_version` が無いことを実測で確認した)。寄せるには要求を 1 回増やすか
  判定を遅らせるかで、どちらも損である。

**「速くなったか」ではなく「答えが変わっていないか」で受け入れた。** 移譲先は
ベース `94618c4` の worktree を作り、**同じランク窓を修正前のコードでも走らせて出力を diff** した。
結果は**追加したサマリ 1 行を除いて完全一致**である。

**その突き合わせが、こちらの期待値の誤りを見つけた。** 移譲時に「`[1]` は 3 件
(digest-crc / debug_inspector / mimemagic)」と渡したが、正しくは 2 件だった —
