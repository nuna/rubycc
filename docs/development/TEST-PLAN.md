...
| 2B fixture job | 完了 | json/msgpackのURL・SHA-256・期待結果をmanifestとCI cache準備処理へ接続し、fetch/unpack/extconf/build/gem installをnetwork namespace内で実行するworkflowを追加 |
...
| 2B-1 fixtureの作成([issue](../../issues/acceptance-fixture-offline.md)) | 完了。`test/fixtures/acceptance/` には期待結果だけを置き、json/msgpack archiveはmanifest URLからCI cacheへ取得してSHA-256を検証するfixture modeを実装。`weekly.yml` はnetwork namespace内でfetch/unpack/extconf/build/gem installを実行する | CI-local archiveのdigest、7つのrequired ID、network遮断下の0 skip/0 failureをローカルで確認済み。gem archiveはリポジトリに含めず、Actions artifactにもアップロードしない |
