...
| M6 以降 | **進行中**。基本最適化の第 1 段(スピルトラフィック削減)が完了し、**C カーネル 5 件では N2 が成立**(gcc -O2 比 1.08〜3.24x)。残りは第 2 段(レジスタ割付)・ヘッダトークンキャッシュ・macOS・行番号デバッグ情報・GCC 擬態モード |
...
| 基本最適化(**第 2 段**: レジスタ割付) | **着手する**([issue](../../issues/register-allocation.md)) | 第 1 段で N2 は満たしたが、**変数そのものはまだスロット往復のまま**。伸びしろは gcc の `-O0 → -O1`(実測 1.04〜4.78x)に相当する分で、`-O1 → -O2` は 1.01〜1.16x しかない(= ベクトル化は主因ではない)。`-O0`/`-O1`/`-O2` の受理と既定レベルの決定を伴う |
...
残るのは第 2 段(レジスタ割付)とヘッダトークンキャッシュである。
