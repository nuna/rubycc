...
検証済み gem は glibc x86-64 が 33 件、musl x86-64 が 3 件、glibc aarch64 が 6 件
...
| ~~マクロ再展開の hide-set 交差~~([issue](../../issues/macro-hide-set-intersection.md)) | **解消(`macro-hide-set-intersection-1`)**: 関数形式マクロの置換 paint を「呼び出し名の suppress ∩ **閉じ括弧の** suppress + 自名」(6.10.3.4 の交差則)へ。呼び出しが 1 つの置換リストから出た場合は両者が等しく交差が恒等なので、自己再帰・相互再帰の停止は不変。c-testsuite 00201 の skip が外れた | ~~起票済み(受け皿だった M2 は Step 54 で完了)~~ **完了** |
