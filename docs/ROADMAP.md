...
現在地: **M1 の途中、Step 16(switch・goto・ラベル文)まで完了**。
...
7. **サンプル**(マイルストーン完了時): その時点でビルドできる C サンプルを
   `examples/m<N>/` に追加して確定する(進行中のマイルストーンにも実装済み機能の
   範囲で随時追加してよい)。全サンプルは `test/test_examples.rb` が gcc 差分で
   常時検証する。運用の詳細は `examples/README.md`。
