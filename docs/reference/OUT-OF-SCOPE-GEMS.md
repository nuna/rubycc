...
| **numo-narray**(x86-64) | G | 型ごとのソース(`t_int8.c` など)を生成する `ext/numo/narray/gen/cogen.rb` が、`RbConfig::CONFIG['target_cpu']` が `x86_64` なら無条件に `is_simd: true` とし、テンプレート `gen/tmpl/lib.c:18` の `#include <emmintrin.h>`(SSE2 の組み込み関数)を生成物に入れる。C の `#if` ではなく生成時の分岐なので、**archive に手を入れずには外せない** | **実測**(2026-09-14、`tools/verify_corpus_candidate.rb`)。rubycc は `t_int8.c:20:1: error: emmintrin.h: No such file or directory` で止まる。ここに至るまでに停止点を 3 つ解消している — `#include` の絶対パス(AO)、旧形式の関数ポインタ(BD)、rmake の生成ソース(BH)。**aarch64 では `is_simd: false` になりスカラーの経路を取るはずだが、測っていない** |
...
| **posix-spawn** | H | `ext/posix-spawn.c:355` の `char *cargv[argc + 1];` — 大きさが `RARRAY_LEN(argv)` で決まる**可変長配列(VLA)** | **実測**(2026-09-14、`tools/verify_corpus_candidate.rb`、ギャップ修正の 4 回目の後)。rubycc は `array size must be an integer constant` で拒否する。AM(`struct sched_param`)を直した後に、この段で止まった。該当行を読んで VLA と確かめてから記録した。**対照の gcc はビルドとロードに成功する**(同日、`--compiler host`、`build_load_pass`) |
