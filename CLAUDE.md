...
**置き場所と書き方の基準は `docs/README.md`**。要点は 2 つ:
`docs/reference/` は**利用者向けに現時点の仕様**(古くなったら直す)、
`docs/development/` は**開発者向けの作業ログ・計画**(古くなったら日付を添えて残す)。
...
- `docs/development/IR.md` — 中間表現の仕様(命令一覧・値表現規約・バックエンドとの契約)。
