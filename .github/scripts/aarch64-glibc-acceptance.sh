...
if [ "${TEST_SCOPE}" = smoke ]; then
  echo "test scope: smoke (twelve native aarch64 acceptance tests; M2 is deferred)"
  ...
  bundle exec ruby -Ilib:test \
    -e 'files = ARGV.shift(12); files.each { |file| require File.expand_path(file) }' \
...
if [ "${TEST_SCOPE}" = smoke ]; then
  CI_MAX_SKIPS=0 CI_MIN_RUNS=12 ruby tools/ci_check_skips.rb \
