...
| `register_allocation_1_promotion.c` | Step register-allocation-1: **C 機能の追加は無い**。x86-64 バックエンドの関数全域レジスタ割付(適格な vreg を最大 5 本、関数の全区間 callee-saved レジスタ rbx/r12〜r15 に固定する)が壊れていないことを差分で守るサンプル。**間違え方は違う数値**(スロットに無い値を読む・呼び出しで壊れたレジスタから読む)なので、ループの back edge を跨ぐ誘導変数とループ不変パラメータ・呼び出しを跨ぐ値(callee-saved 性と ret ごとの復元。main 自身が呼び出しを跨ぐ値を持つ)・アドレスを取った変数(ポインタ経由の書き込みが変数の読みに見えること)・三重ループ(出現の重み付け)・レジスタ数を超える 7 値(あふれた分がスロットに残っても正しいこと)・2 つの return(どちらの経路でも復元)を網羅する。`test_examples.rb` が gcc 差分で検証する |
| `register_allocation_2_operands.c` | Step register-allocation-2: **C 機能の追加は無い**。昇格レジスタを**命令のオペランドとして名指す**(r/m フィールド・`lea` のベース/インデックス・結果を書き込む先)ことが壊れていないことを差分で守るサンプル。**間違え方は違う数値**(オペランドの順序違い・SIB の特殊符号化による誤ったアドレス・上位 32 ビットの残留)なので、ベースとインデックスが両方昇格した添字(**SIB の 3 つの罠**: `r13` ベースは「ベース無し + disp32」と衝突するので disp8=0 が要る・`r12` ベースは SIB ベース 100 + REX.B・`r12` インデックスは REX.X が無いと「インデックス無し」)・第 1 オペランドが自分自身の累算(int と long の両方。int は上位半分が消えること)・可換演算で累算器が第 2 オペランドに来る形・非可換演算の右辺に来る昇格値(減算・シフト・除算・剰余・比較)・符号付き/符号無しの除算と剰余の除数・1/2/4 バイトからの符号拡張とゼロ拡張・昇格値を条件とする分岐(ポインタ含む)・呼び出しを跨ぐ同じ形を網羅する。`test_examples.rb` が gcc 差分で検証する |
...
| `warning_directive_1_notice.c` | Step warning-directive-1: `#warning`(C23 6.10.2p2、長く GNU 拡張)とそれが必要とした**警告チャネル**。**ビルドそのものが実演対象**で、gcc でも rubycc でも標準エラーに警告が出たうえで終了コード 0・オブジェクト生成に至る(標準出力は汚さない)。無条件の `#warning`・引用符あり/なしの両綴り(引用符は必須ではなく、行の残り全体がメッセージ)・**読み飛ばされる条件グループ内**の `#warning`(報告されない)・生きた条件の `#else` で踏む`#warning`(「未知のコンパイラ」を告げて保守的な既定へ倒す移植性ヘッダの形)を網羅する。実行時出力はその「未知」分岐が選んだ値なので、`test_examples.rb` の gcc 差分(終了コード + 標準出力)が警告を出したビルドの通った経路を覆う |
| `source_encoding_1_non_ascii.c` | Step default-external-encoding-1: **C 機能の追加は無い**。ソースを**バイト列**として読むこと(C の字句解析は 5.1.1.2 のとおりバイト列に対して行う)が壊れていないことを差分で守るサンプル。以前はソースの読み出しが `Encoding.default_external` に依存しており、**ロケール未設定の環境では同梱ヘッダのコメント中の非 ASCII バイトで即死**し(`#include <stddef.h>` だけの TU がコンパイルできなかった)、**UTF-8 文字列リテラルはロケールに関係なく壊れていた**(文字を符号位置として扱っていたため `12354 out of char range`)。UTF-8 のブロックコメントと行コメント・UTF-8 文字列リテラルの出力・同じ内容を 16 進エスケープで綴った文字列とのバイト単位の比較・`sizeof`(要素は文字ではなく**バイト**)・各バイトの添字取り出し・隣接 UTF-8 リテラルの連結(翻訳フェーズ 6)を網羅する。識別子は意図的に ASCII のみ(基本文字集合外の識別子は別の主題)。**間違え方は生バイトの不一致**なので、`test_examples.rb` の gcc 差分(終了コード + 標準出力)がそのまま検証になる |
| `extern_initializer_file_scope_1_definition.c` | Step extern-initializer-file-scope-1: ファイルスコープの **`extern` 付き定義**(初期化子あり)。C11 6.9.2p1 の例のとおり外部定義であり、`extern` が無い場合と同じに振る舞う。スカラー・構造体・構造体へのポインタ(cool.io 同梱 libev の `EV_API_DECL struct ev_loop *ev_default_loop_ptr = 0;` の形)を定義し、読み書きする。gcc は警告を出して通す。別の翻訳単位から参照する形は 1 ファイルでは実演できないので、`test/test_extern_initializer_file_scope.rb` の gcc 差分が担う。`test_examples.rb` が gcc 差分で検証する |
| `stdc_no_vla_macro_1_array_param.c` | Step stdc-no-vla-macro-1: **`__STDC_NO_VLA__`**(C11 6.10.8.3)。VLA に対応しない処理系が定義しなければならないこのマクロを、rubycc が定義するようになったことを実演する。コーパス候補 `brotli` 0.8.0 の `vendor/brotli/c/include/brotli/port.h:257-263` と同じ分岐(マクロが定義されていれば配列仮引数の `[]` を空にし、VLA 仮引数の形を避ける)を再現し、マクロが無ければ rubycc が拒否していた形(`array size must be an integer constant`)を回避できることを示す。`test_examples.rb` が gcc 差分で検証する |
| `escape_sequence_e_1_ansi_color.c` | Step escape-sequence-e-1: GNU 拡張のエスケープ `\e` / `\E`(ESC、0x1B)。C11 6.4.4.4 の単純エスケープには無い綴りだが、gcc は既定で受理する(`-pedantic` でのみ警告)。文字列リテラル・文字定数・wide 文字定数(`L'\e'`)の 3 箇所で使い、いずれも 0x1B になることを実行して確かめる。コーパス候補 `string_undump` 0.1.1 の `return "\e";` がこの形。`test_examples.rb` が gcc 差分で検証する |
| `extension_struct_member_1_member_prefix.c` | Step extension-struct-member-1: 構造体・共用体のメンバ宣言の頭に付く `__extension__`(GNU マーカー、意味は持たない)。パーサは宣言の頭に並ぶ `__extension__` を外部宣言・ブロックスコープ宣言では読み飛ばしていたが、構造体・共用体のメンバ宣言を読む経路では読み飛ばしていなかった(`expected type specifier`)。構造体メンバ・共用体メンバ・入れ子構造体のメンバの 3 形に前置し、`sizeof`/`offsetof` が gcc と一致することを実演する。glibc の `<threads.h>` が読み込む `bits/atomic_wide_counter.h:27` がこの形の宣言を持っており、この修正で `<threads.h>` が rubycc でコンパイルできるようになった(`__STDC_NO_THREADS__` の予約定義はこのステップで撤回)。`test_examples.rb` が gcc 差分で検証する |
| `attribute_statement_1_fallthrough.c` | Step attribute-statement-1: 文として書いた GNU 属性 `__attribute__((fallthrough));`。宣言の頭の属性(`__attribute__((unused)) int x;`)と違い、直後に `;` が来る空文としての属性を実演する。gcc は `fallthrough` という名前だけを特別扱いし、次が `case`/`default` でないと診断するが、rubycc は DESIGN R7 のとおり `aligned`/`packed` 以外の属性に意味を持たせないので、`fallthrough` を含めどの属性名でも構文として受理し無視する(位置の妥当性は診断しない)。未知の属性名の文・宣言頭の属性(回帰確認)も同じファイルで実演する。`liquid-c` 4.2.0(`ext/liquid_c/parser.c:242`)がこの形を書いていた。`test_examples.rb` が gcc 差分で検証する |
| `builtin_strlen_1_constant_fold.c` | Step builtin-strlen-1: `__builtin_strlen`。文字列リテラル引数を gcc がコンパイル時の定数に畳むこと(配列の大きさ・静的初期化子・`_Static_assert`・`case` ラベルの 4 箇所、いずれも genuine な constant-expression として通る — 2026-09-13、gcc 13.3 実測)と、`char *` 変数引数では通常の実行時呼び出しになること(libc の `strlen` と同じ値)を実演する。埋め込み `| `attribute_statement_1_fallthrough.c` | Step attribute-statement-1: 文として書いた GNU 属性 `__attribute__((fallthrough));`。宣言の頭の属性(`__attribute__((unused)) int x;`)と違い、直後に `;` が来る空文としての属性を実演する。gcc は `fallthrough` という名前だけを特別扱いし、次が `case`/`default` でないと診断するが、rubycc は DESIGN R7 のとおり `aligned`/`packed` 以外の属性に意味を持たせないので、`fallthrough` を含めどの属性名でも構文として受理し無視する(位置の妥当性は診断しない)。未知の属性名の文・宣言頭の属性(回帰確認)も同じファイルで実演する。`liquid-c` 4.2.0(`ext/liquid_c/parser.c:242`)がこの形を書いていた。`test_examples.rb` が gcc 差分で検証する |
| `unprototyped_function_pointer_compat_1_struct_member.c` | Step unprototyped-function-pointer-compat-1: プロトタイプ付き関数ポインタ(`void (*)(int *, long)`)と旧形式の関数ポインタ(`void (*)()`、空の識別子リスト、関数定義の外)の互換(C11 6.7.6.3p15)。構造体メンバに旧形式で宣言した関数ポインタへプロトタイプ付きの値を代入する形(逆方向・実引数として渡す形も同じ関数内で実演)を、`numo-narray` 0.9.2.1 の `ext/numo/narray/ndloop.c`(`na_md_loop_t` の `void (*loop_func)();`)に倣って再現する。仮引数の型が既定の実引数拡張(6.5.2.2p6)で変わらない(`int *`/`long`)ことが互換の条件で、`char`/`short`/`float` 仮引数や省略記号を持つプロトタイプは対象外(`test_unprototyped_function_pointer_compat.rb` が診断されることを確認)。`test_examples.rb` が gcc 差分で検証する |
| `unprototyped_function_redeclaration_1_old_style_header.c` | Step unprototyped-function-redeclaration-1: 旧形式(`int area();`、空の `()`)で宣言した名前付き関数を、プロトタイプ付きの定義より前に引数付きで呼び、後で仮引数付きで再宣言・定義する形(C11 6.7.6.3p15・6.2.7p3)。古いヘッダの `int f();` 形の宣言を模す。旧形式の宣言しか見えていない呼び出しでは実引数に既定の実引数拡張(6.5.2.2p6)がかかり、`char` は `int`、`float` は `double` として渡る。`int f(); int f(char);` のように拡張で型が変わる仮引数や省略記号を持つプロトタイプとの組み合わせは対象外(`test_unprototyped_function_redeclaration.rb` が gcc と同じく `conflicting types` になることを確認)。`test_examples.rb` が gcc 差分で検証する |
| `attribute_statement_after_label_1_case_label.c` | Step attribute-statement-after-label-1: ラベルの**直後の 1 文**として書いた GNU 属性 `__attribute__((fallthrough));`(`attribute_statement_1_fallthrough.c` と違い、実文を経由せず `case`/`default`/通常のラベルの直後に直接置く形)。`attribute-statement-1` は `#parse_block_item` に届く位置だけを直しており、`#parse_nested_statement` から `#parse_statement` を直接呼ぶこの経路には届いていなかった(issue "attribute-statement-after-label"、GAPS BE)。`case N:` の直後・`default:` の直後・通常の(goto ターゲットの)ラベルの直後の 3 形を実演する。`test_examples.rb` が gcc 差分で検証する |
| `predefined_identifier_func_1_predefined_identifiers.c` | Step predefined-identifier-func-1: 予約識別子 `__func__`(ISO C99 6.4.2.2)と GNU 別名 `__FUNCTION__`/`__PRETTY_FUNCTION__`。C モードでは 3 綴りとも装飾なしの関数名になる(gcc 実測)ことと、`sizeof`(名前のバイト数 + NUL)・入れ子ブロックでの利用・関数ごとに追従すること・1 関数内の複数参照(`&` 経由も含む)が同一オブジェクトになることを実演する。OpenSSL 3 の `OPENSSL_FUNC` マクロがこの綴りへ展開し、trilogy 2.13.0 のビルドがこれを踏んでいた。`test_examples.rb` が gcc 差分で検証する |
| `variadic_aggregate_argument_1_semctl_shape.c` | Step variadic-aggregate-argument-1: 可変長引数の位置に構造体・共用体を**値で**渡し、呼ばれ側が `va_arg(ap, struct T)` で読む。semctl(2) の形(名前付き int 3 個の後に `union semun` 相当を値で、`semian` 0.28.4 の `sysv_semaphores.c:90` と同じ)・8 バイト以下の共用体・9〜16 バイトの整数構造体・double 2 個(x86-64 は SSE 2 個、AArch64 は HFA)・float 2 個・int と double の混在・16 バイト超(x86-64 はスタック、AArch64 はコピーへのポインタ)を、タグで種類を選ぶ読み手と、整数・ベクタ両レジスタの上限を越える前置きの後ろで読む形の両方で実演する。`test_examples.rb` が gcc 差分で検証する |
| `atomic_builtin_small_widths_1_byte_spinlock.c` | Step atomic-builtin-small-widths-1: 1・2 バイトのオブジェクトへのアトミック組み込み関数と、ビット演算の fetch 形。facil.io の `fio_lock_i` と同じ形の 1 バイトスピンロック(`__atomic_exchange_n` で取り、`__atomic_store_n` で放す)が守る普通のカウンタを 4 スレッドで増やし、同時に 1 バイトの `__atomic_fetch_add`・2 バイトの `__atomic_sub_fetch`・共有フラグバイトへの `__atomic_fetch_or` / `__atomic_and_fetch` を回して、合計がすべて既知の値にちょうど一致すること(更新が失われたりちぎれたりしていないこと)を示す。続く単一スレッド部で、結果がオブジェクトの型どおりに拡張されること(`unsigned char` の 0xF0 は 240、`signed char` の同じビットは -16)、`signed char` への compare-exchange が期待値 -1 と 0xFF を一致とみなすこと、`unsigned short` の 65535 + 2 が 1 に巻き戻ることを示す。これが通らなかったため iodine 0.7.59 のビルドが fio.h:3023 で止まっていた。`test_examples.rb` が gcc 差分で検証する |
| `function_definition_parenthesized_name_1_dodge_macro.c` | Step function-definition-parenthesized-name-1: 関数名を丸括弧で包んだ宣言子からの関数定義(同名の関数マクロを避ける定石を含む) |
| `bundled_headers_coverage_audit_2_glibc_surface.c` | Step bundled-headers-coverage-audit-2: 同梱ヘッダの監査で足した名前を、足りなかった gem の使い方で実演する。`qsort_r`(文脈ポインタ付きの GNU 順)と `getloadavg`(`<stdlib.h>`)、`struct sched_param` と `SCHED_*`(`<sched.h>`)、`TCOOFF`〜`TCION`・`B115200`・`CRTSCTS`・`cfsetspeed`(`<termios.h>`)、`TIOCMGET`・`TIOCM_DTR`・`SIOCGIFHWADDR`(`<sys/ioctl.h>`)、glibc の内部名 `__caddr_t`・`__pid_t` と `int32_t`(`<sys/types.h>`)。出力は ABI で決まる値だけで、端末や負荷には依らない。`alloca` は aarch64 の例の実行系が glibc 本体の `<alloca.h>`(`__GNUC__` のときだけ組み込みに写す)を読むため入れず、`test_header_abi.rb` の `STDLIB_GNU` で確かめる。`test_examples.rb` が gcc 差分で検証する |
| `aapcs64_aligned_attribute_aggregate_1_attribute_vs_member.c` | Step aapcs64-aligned-attribute-aggregate-1: 型の属性で 16 バイト整列にした構造体とメンバで 16 バイト整列にした構造体を、奇数個の long の後・レジスタを使い切った後に、固定引数・可変長引数・戻り値で渡す(AArch64 では前者だけ偶数レジスタに切り上げない) |
...
| `overaligned_automatic_object_1_cache_line_scratch.c` | Step overaligned-automatic-object-1: フレームより強く整列した局所オブジェクト(64 バイトの scratch・32 整列の構造体・16 整列のカウンタ)を、スタック引数・可変長引数・`__builtin_alloca` と併せて再整列したフレームに置く |
| `glibc_alloca_without_gnuc_1_stack_blocks.c` | Step glibc-alloca-without-gnuc-1: libc の名前そのままの `alloca` 呼び出し。同梱の `<alloca.h>` は名前を無条件に `__builtin_alloca` へ写すが、glibc 本体の `<alloca.h>` は `__GNUC__` のときだけ写す(rubycc は定義しない — DESIGN R7)ため、そちらを読む経路では `alloca` という関数の呼び出しが残り、どの libc にもその名前のシンボルは無いのでリンクできなかった(2026-09-18 実測、x86-64・aarch64 とも `undefined reference to 'alloca'`)。実行時サイズのブロック・同時に生きる 2 ブロック・ループ内で毎回確保して関数の出口でまとめて解放されるブロック・別関数へ渡すブロックを実演し、いずれも 16 バイト境界にあることを示す。ホストの実行系は同梱ヘッダを、aarch64 の実行系はクロス sysroot の glibc 本体のヘッダを読むので、1 つのファイルで両方の経路を通る。`test_examples.rb` / `test_examples_aarch64.rb` が gcc 差分で検証する |
