...
| byebug | 13.0.0 | 13.0.0 | ok | 5/1 | 470,544,259 downloads. Single ext dir (ext/byebug); extconf.rb is 12 lines and only calls dir_config. C 5 files / H 1 file. |
...
| thin | 2.0.1 | 2.0.1 | ok | 2/2 | 207,539,292 downloads. Single ext dir (ext/thin_parser), a Ragel-generated parser: C 2 files / H 2 files. Note: thin's own extension is pure C, but its runtime dependency eventmachine is a C++ extension, so `gem install thin` additionally requires building eventmachine; the census here only looks at thin's own C sources. |
...
| unicorn | 6.1.0 | 6.1.0 | ok | 2/5 | 118,284,401 downloads. Single ext dir (ext/unicorn_http); C 2 files / H 5 files, extconf.rb has no probes. Note: its dependencies kgio and raindrops are also C extensions, so `gem install unicorn` additionally requires building those. |
| debug | 1.11.1 | 1.11.1 | ok | 2/0 | 116,172,789 downloads. Single ext dir (ext/debug); C 2 files, extconf.rb 27 lines with no probes. Ruby's standard debugger. |
...
| header | class | json | msgpack | bigdecimal | date | racc | redcarpet | digest | erb | etc | io-console | io-nonblock | io-wait | openssl | prism | psych | stringio | strscan | zlib | fiddle | rbs | syslog | websocket-driver | puma | google-protobuf | bootsnap | oj | nio4r | byebug | mysql2 | thin | http_parser.rb | stackprof | unicorn | debug | yajl-ruby | nkf |
|--------|-------|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
