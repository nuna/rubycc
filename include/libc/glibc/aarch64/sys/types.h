/* rubycc bundled <sys/types.h>: the width-critical POSIX typedefs. Derived from
   ...
   2026-09-14, run on x86-64). bundled-headers-coverage-audit-2 therefore
   added the whole set of scalar __*_t names glibc's <sys/types.h> shows,
   plus int8_t..int64_t and quad_t/u_quad_t. Each width, alignment and
   signedness was measured on both arches on 2026-09-14 (test/
   test_header_abi.rb's SYS_TYPES case); as with the public names, __nlink_t
   and __blksize_t are 32-bit here where x86-64 makes them 64-bit. Every one
   ...
   (measured 2026-09-14 on x86-64 when it was tried: <aio.h>, <mqueue.h>,
