...
module Rubycc
  module ObjFile
    ...
    #
    # Section layout (in this order): NULL, .text, .rela.text (only when there
    # are relocations), .note.GNU-stack, .symtab, .strtab, .shstrtab. Section
    # indices are not hard-coded: the ordered name list is assembled in
    # #to_binary and a name -> index lookup resolves the cross-references
    # (symtab's sh_link, rela's sh_link/sh_info, and each symbol's st_shndx).
    # Symbol table order is NULL, STT_FILE, the .text section symbol, then the
    # global symbols (defined functions followed by undefined externals);
    # r_info in .rela.text indexes into that final order.
    #
    ...
    class ELFWriter
      ...
      # x86_64 relocation type used for near calls (PC-relative, PLT-aware).
      R_X86_64_PLT32 = 4
      ...
      def add_text_relocation(offset:, symbol:)
        @relocations << { offset: offset, symbol: symbol }
        self
      end
      ...
      def to_binary
        ...
        rela = relocations? ? build_rela(symbol_indices) : nil
      ...
      # .rela.text only appears when there is something to relocate.
      ...
      def build_symtab(symbols, sym_name_offsets)
        ...
        symbols.each do |sym|
          buf << sym_entry(
            ...
            other: 0,
            shndx: sym[:shndx] == :text ? section_index(:text) : sym[:shndx],
            value: sym[:value],
            size: sym[:size]
          )
        end
      ...
      # Builds the .rela.text payload: one entry per call site, each pointing at
      # its symbol with R_X86_64_PLT32 and an addend of -4 (the PC-relative
      # bias for a "call rel32").
      def build_rela(symbol_indices)
        buf = +"".b
        @relocations.each do |reloc|
          sym_index = symbol_indices.fetch(reloc[:symbol])
          r_info = (sym_index << 32) | R_X86_64_PLT32
          buf << [reloc[:offset]].pack("Q<")
          buf << [r_info].pack("Q<")
          buf << [-4].pack("q<")
        end
        buf
